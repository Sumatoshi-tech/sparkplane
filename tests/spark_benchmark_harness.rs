...
fn live_run_parses_fragmented_sse_without_retaining_generated_text() {
    ...
    std::fs::write(temp.path().join("observations.json"), r#"{"schema":"sparkplane.benchmark-observations/v1","native":{"mtp_acceptance_rate":0.75},"resources":{"mem_available_bytes":10000000000},"lifecycle":{"healthy":true}}"#).unwrap();
    let output = Command::new("python3")
        .args([
            "scripts/benchmark-spark-engine.py",
            "--fixture",
...
fn comparison_rejects_missing_immutable_identity() {
    let temp = tempfile::tempdir().unwrap();
    let incomplete = r#"{"schema":"sparkplane.benchmark-result/v1","mode":"live","samples":[]}"#;
    let left = temp.path().join("left.json");
    let right = temp.path().join("right.json");
...
fn live_run_rejects_incomplete_immutable_metadata_before_connecting() {
    ...
    std::fs::write(temp.path().join("observations"), r#"{"schema":"sparkplane.benchmark-observations/v1","native":{},"resources":{},"lifecycle":{}}"#).unwrap();
    let output = Command::new("python3")
        .args([
            "scripts/benchmark-spark-engine.py",
            "--fixture",
