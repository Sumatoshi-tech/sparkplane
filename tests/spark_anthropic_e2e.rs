...
fn malformed_pairing_oversize_image_and_unknown_content_fail_before_upstream() {
    ...
    assert!(
        gateway::rewrite_anthropic_request(
...
#[test]
#[ignore = "set SPARKPLANE_CLAUDE_BIN to the pinned Claude Code 2.1.241 binary"]
fn pinned_claude_code_2_1_241_completes_one_streamed_client_tool_round_trip() {
    let binary = std::env::var("SPARKPLANE_CLAUDE_BIN").expect("exact Claude Code binary path");
