...
use axum::{
    ...
    body::{Body, Bytes},
    extract::{
        ConnectInfo, DefaultBodyLimit, Extension, Path as AxumPath, Query, RawQuery, Request,
        State, rejection::BytesRejection,
    },
...
use super::wire::{
    AnthropicErrorDetail, AnthropicErrorDocument, AnthropicTokenCountDocument, CERTIFICATE_SCHEMA,
    CertificateStatusDocument, DOCTOR_SCHEMA, DatabaseHealth, DegradedReason, DoctorCheck,
    DoctorDocument, DownloadPlanDocument, DownloadRequest, ENGINE_LOG_SCHEMA, EngineLogDocument,
    INSTANCE_LIST_SCHEMA, INSTANCE_SCHEMA, InstanceDesiredState, InstanceDocument,
    InstanceListDocument, InstanceObservedState, MODEL_LIST_SCHEMA, ModelDocument,
    ModelListDocument, OPERATION_LIST_SCHEMA, OpenAiEmbeddingDocument, OpenAiEmbeddingRequest,
    OpenAiEmbeddingUsage, OpenAiEmbeddingVector, OperationDocument, OperationListDocument,
    OperationProgress, PROBLEM_SCHEMA, ProblemDocument, REMOVAL_PLAN_SCHEMA, RemovalPlanDocument,
    RemoveModelRequest, STATUS_SCHEMA, ServeAdmissionRequest, ServeRequest, StatusDocument,
    StopRequest, TOKEN_LIST_SCHEMA, TokenCreateRequest, TokenCreatedDocument, TokenListDocument,
    TokenScope,
};
use super::{
    executor::{
        CandidateStorage, CandidateStorageInput, ExecutorClient, ExecutorClientError, LogInput,
        ObservedEngine, ReconcileExpectation, StartInstanceInput, StopInstanceInput,
    },
    ...
    reconcile::{DesiredIntent, ReconcileAction, ValidatedObservation, decide},
    resources::{
        AdmissionRequest, CandidateEnvelope, DeclaredEnvelope, ResourcePolicyConfig,
        TransitionCoordinator, TransitionLease, TransitionLeaseError, evaluate_admission,
        persistent_set_fits_reboot_envelope, unix_millis,
    },
...
const API_BASE: &str = "/api/sparkplane/v1";
...
const QUALIFICATION_CANCEL_REGISTRATION_ATTEMPTS: usize = 32;
const QUALIFICATION_CANCEL_REGISTRATION_INTERVAL: Duration = Duration::from_millis(10);
...
impl AgentState {
    ...
    fn with_qualification_authority(mut self, authority: impl Into<Arc<str>>) -> Self {
        self.qualification_authority = Some(authority.into());
        self
    }
...
#[openapi(
    ...
    components(schemas(
        ...
        crate::spark::qualification::QualificationSubmission,
        crate::spark::qualification::QualificationPlanDocument,
        crate::spark::qualification::QualificationResultDocument
    ))
)]
struct ApiDoc;
...
pub fn router(state: AgentState) -> Router {
    let authenticated = Router::new()
        ...
        .route(
            &format!("{API_BASE}/qualifications"),
            axum::routing::post(qualification),
        )
...
#[utoipa::path(post, path = "/api/sparkplane/v1/admission", request_body = ServeAdmissionRequest, responses((status = 200, body = crate::spark::resources::AdmissionReport)))]
async fn admission(
    State(state): State<AgentState>,
...
#[utoipa::path(post, path = "/api/sparkplane/v1/instances", request_body = ServeRequest, responses((status = 202, body = OperationDocument)))]
...
async fn run_serve(
    ...
    .map(|route| route.for_process(observed.init_pid, observed.pid_start_time_ticks))
    .and_then(|route| {
...
async fn reconcile_running_engine(
    ...
    if observed.running
        ...
        && routes.is_some_and(|routes| {
            exact_route_is_published(
                routes,
                instance,
                (observed.init_pid, observed.pid_start_time_ticks),
            )
        })
    {
        return Ok(());
    ...
    .map_err(|_| ())?
    .for_process(observed.init_pid, observed.pid_start_time_ticks);
    let request = route
        .request(&observed.health_method, &observed.health_path, 0)
        .map_err(|_| ())?;
    let deadline = Duration::from_secs(observed.startup_deadline_seconds);
...
fn exact_route_is_published(
    ...
    matches!(
        ...
        RouteLookup::Healthy(route) if route.generation == instance.generation && route.upstream.matches_process(process)
    )
}
...
async fn fail_serve(
    ...
    let problem = ProblemDocument {
        ...
        r#type: "https://sparkplane.local/problems/spark-engine-start".into(),
...
#[utoipa::path(get, path = "/api/sparkplane/v1/instances", responses((status = 200, body = InstanceListDocument)))]
...
#[utoipa::path(delete, path = "/api/sparkplane/v1/instances/{id}", params(("id" = String, Path)), request_body = StopRequest, responses((status = 202, body = OperationDocument)))]
...
async fn run_stop(
    ...
    match stopped {
        ...
        false => {
            let problem = ProblemDocument {
                schema: PROBLEM_SCHEMA.into(),
                r#type: "https://sparkplane.local/problems/spark-engine-stop".into(),
...
#[utoipa::path(get, path = "/api/sparkplane/v1/instances/{id}/logs", params(("id" = String, Path), ("cursor" = Option<u64>, Query), ("limit" = Option<usize>, Query)), responses((status = 200, body = EngineLogDocument)))]
...
#[utoipa::path(get, path = "/api/sparkplane/v1/models", responses((status = 200, body = ModelListDocument)))]
...
#[utoipa::path(get, path = "/api/sparkplane/v1/models/{id}", params(("id" = String, Path)), responses((status = 200, body = ModelDocument)))]
async fn get_model(
    State(state): State<AgentState>,
    AxumPath(id): AxumPath<String>,
    RawQuery(query): RawQuery,
...
#[utoipa::path(post, path = "/api/sparkplane/v1/downloads", request_body = DownloadRequest, responses((status = 200, body = DownloadPlanDocument), (status = 202, body = OperationDocument)))]
    ...
    if body.dry_run {
            ...
            .await
        {
            Ok(plan) => Json(DownloadPlanDocument {
                schema: "sparkplane.download-plan/v1".into(),
...
#[utoipa::path(delete, path = "/api/sparkplane/v1/models/{id}", params(("id" = String, Path)), request_body = RemoveModelRequest, responses((status = 200, body = RemovalPlanDocument), (status = 202, body = OperationDocument)))]
...
#[utoipa::path(get, path = "/api/sparkplane/v1/status", responses((status = 200, body = StatusDocument)))]
...
#[utoipa::path(get, path = "/api/sparkplane/v1/metrics", responses((status = 200, body = String, content_type = "text/plain")))]
async fn metrics(State(state): State<AgentState>, RawQuery(query): RawQuery) -> Response {
    ...
    let body = format!(
        "# TYPE sparkplane_agent_up gauge\nsparkplane_agent_up 1\n# TYPE sparkplane_executor_ready gauge\nsparkplane_executor_ready {}\n# TYPE sparkplane_operations_total gauge\nsparkplane_operations_total {operations}\n# TYPE sparkplane_instances_total gauge\nsparkplane_instances_total {instances}\n# TYPE sparkplane_models_total gauge\nsparkplane_models_total {models}\n",
...
#[utoipa::path(get, path = "/api/sparkplane/v1/operations/{id}", params(("id" = String, Path)), responses((status = 200, body = OperationDocument)))]
...
#[utoipa::path(get, path = "/api/sparkplane/v1/operations/{id}/events", params(("id" = String, Path)), responses((status = 200, description = "resumable operation event stream")))]
...
#[utoipa::path(delete, path = "/api/sparkplane/v1/operations/{id}", params(("id" = String, Path)), responses((status = 202, body = OperationDocument)))]
async fn cancel_operation(
    ...
    let current = match database.operation(&id).await {
        Ok(operation) => operation,
        Err(error) => return state_problem(error),
    };
    if auth.id != "bootstrap-admin" && auth.id != current.actor_token_id {
        return problem(
            StatusCode::FORBIDDEN,
            "spark.operation.owner-required",
            "only the operation owner or bootstrap administrator may cancel it",
        );
    }
    if current.state.is_terminal() {
        return problem(
            StatusCode::CONFLICT,
            "spark.operation.terminal",
            "terminal operations are immutable",
        );
    }
    if current.kind == "qualification.run" && current.state == super::wire::OperationState::Running
    {
        let Some(executor) = state.executor.clone() else {
            return executor_unavailable();
        };
        let operation_id = match ulid::Ulid::from_string(&current.id) {
            Ok(operation_id) => operation_id,
            Err(_) => {
                return problem(
                    StatusCode::SERVICE_UNAVAILABLE,
                    "spark.qualification.operation-invalid",
                    "durable qualification operation identity is invalid",
                );
            }
        };
        let mut acknowledged = false;
        for attempt in 0..QUALIFICATION_CANCEL_REGISTRATION_ATTEMPTS {
            match executor.cancel_qualification(operation_id).await {
                Ok(true) => {
                    acknowledged = true;
                    break;
                }
                Ok(false) if attempt + 1 < QUALIFICATION_CANCEL_REGISTRATION_ATTEMPTS => {
                    tokio::time::sleep(QUALIFICATION_CANCEL_REGISTRATION_INTERVAL).await;
                }
                Ok(false) => break,
                Err(error) => return qualification_executor_problem(error),
            }
        }
        if !acknowledged {
            return problem(
                StatusCode::CONFLICT,
                "spark.qualification.not-active",
                "executor did not acknowledge an active qualification request",
            );
        }
    }
    match database.cancel(&id, &auth.id).await {
        Ok(operation) => {
            if operation.kind == "instance.serve"
                && let (Some(target), Some(executor)) =
                    (operation.target.as_deref(), state.executor.clone())
                && let Ok(instance) = database.instance(target).await
                && instance.observed != InstanceObservedState::Healthy
            {
...
#[utoipa::path(
    post,
    path = "/api/sparkplane/v1/qualifications",
    request_body = crate::spark::qualification::QualificationSubmission,
    responses(
        (status = 200, body = crate::spark::qualification::QualificationPlanDocument),
        (status = 202, body = OperationDocument),
        (status = 400, body = ProblemDocument),
        (status = 409, body = ProblemDocument)
    )
)]
async fn qualification(
    ...
    headers: HeaderMap,
    Extension(auth): Extension<AuthenticatedToken>,
    Json(submission): Json<super::qualification::QualificationSubmission>,
) -> Response {
    let key = match required_idempotency(&headers) {
        Ok(key) => key,
        Err(()) => return missing_idempotency(),
    ...
    let Some(authority) = state.qualification_authority.clone() else {
        return problem(
            StatusCode::SERVICE_UNAVAILABLE,
            "spark.qualification.authority-unavailable",
            "qualification release authority is unavailable",
        );
    };
    let plan = match executor.qualification_plan(submission.clone()).await {
        Ok(plan) => plan,
        Err(error) => return qualification_executor_problem(error),
    };
    let mut independently_verified =
        match super::qualification::verify_submission(&submission, &authority, &plan.target) {
            Ok(verified) => verified.plan,
            Err(_) => {
                return problem(
                    StatusCode::BAD_REQUEST,
                    "spark.qualification.signature-invalid",
                    "qualification manifest signature or identity is invalid",
                );
            }
        };
    independently_verified.resources = plan.resources.clone();
    if independently_verified != plan {
        return problem(
            StatusCode::SERVICE_UNAVAILABLE,
            "spark.qualification.executor-mismatch",
            "executor qualification verification differs from the agent",
        );
    }
    ...
    let transition = match state
        .admission
        .try_acquire(format!("qualification:{}", plan.job_id))
    {
        Ok(lease) => lease,
        Err(TransitionLeaseError::Busy { .. }) => {
            return problem(
                StatusCode::CONFLICT,
                "spark.qualification.transition-busy",
                "another high-memory transition owns the admission lease",
            );
        }
        Err(TransitionLeaseError::Unavailable) => {
            return problem(
                StatusCode::SERVICE_UNAVAILABLE,
                "spark.qualification.admission-unavailable",
                "qualification admission coordinator is unavailable",
            );
        }
    };
    let request_hash = match super::wire::canonical_request_sha256(&submission) {
        Ok(hash) => hash,
        Err(_) => {
            return problem(
                StatusCode::BAD_REQUEST,
                "spark.qualification.request-invalid",
                "qualification request cannot be canonicalized",
            );
        }
    };
    let accepted = match database
    ...
    if !accepted.reused {
        tokio::spawn(run_qualification(
            database,
            executor,
            transition,
            accepted.operation.id.clone(),
            submission,
            plan,
        ));
    }
...
async fn run_qualification(
    database: DbActor,
    executor: ExecutorClient,
    _transition: TransitionLease,
    operation_id: String,
    submission: super::qualification::QualificationSubmission,
    admitted_plan: super::qualification::QualificationPlanDocument,
) {
    let running = database
        .transition(
            &operation_id,
            ...
            OperationProgress {
                ...
                total: None,
                unit: None,
                message: "running signed one-shot GPU qualification".into(),
            },
            None,
            None,
        )
    ...
    let deadline = Duration::from_secs(admitted_plan.limits.timeout_seconds);
    let input = super::qualification::QualificationRunInput {
        operation_id: operation_id.clone(),
        submission,
    };
    match executor.run_qualification(input, deadline).await {
        Ok((plan, execution)) if qualification_plan_identity_matches(&admitted_plan, &plan) => {
            if execution.validate_output_identity().is_err() {
                fail_qualification(
                    &database,
                    &operation_id,
                    "spark.qualification.output-invalid",
                    "executor returned qualification output with an invalid content identity",
                )
                .await;
                return;
            }
            let passed = execution.exit_code == 0;
            let result = super::qualification::QualificationResultDocument::new(plan, execution);
            let result_value = serde_json::to_value(&result).ok();
            let problem = (!passed).then(|| {
                qualification_failure_document(
                    &operation_id,
                    "spark.qualification.failed",
                    "qualification runner returned a non-zero exit code",
                )
            });
            let _ = database
                .transition(
                    ...
                    OperationProgress {
                        stage: if passed { "complete" } else { "failed" }.into(),
                        current: None,
                        total: None,
                        unit: None,
                        message: if passed {
        ...
        Ok(_) => {
            fail_qualification(
                ...
                &operation_id,
                "spark.qualification.executor-mismatch",
                "executor changed immutable qualification identity",
            )
            .await;
        }
        Err(error) if error.code == sparkplane_core::ErrorCode::Cancelled => (),
        Err(error) => {
            if database
                .operation(&operation_id)
                .await
            ...
            let (code, detail) = qualification_terminal_failure(error);
            fail_qualification(&database, &operation_id, code, detail).await;
        }
    }
}

fn qualification_terminal_failure(error: ExecutorClientError) -> (&'static str, &'static str) {
    match error.code {
        sparkplane_core::ErrorCode::PolicyDenied => (
            "spark.qualification.output-limit",
            "qualification output exceeded its signed transport limit",
        ),
        sparkplane_core::ErrorCode::Timeout => (
            "spark.qualification.timeout",
            "qualification exceeded its signed deadline",
        ),
        _ => ("spark.qualification.executor-failed", error.detail),
    }
}

fn qualification_plan_identity_matches(
    admitted: &super::qualification::QualificationPlanDocument,
    executed: &super::qualification::QualificationPlanDocument,
) -> bool {
    let mut admitted = admitted.clone();
    admitted.resources = executed.resources.clone();
    admitted == *executed
}

async fn fail_qualification(database: &DbActor, operation_id: &str, code: &str, detail: &str) {
    let problem = qualification_failure_document(operation_id, code, detail);
    let _ = database
        .transition(
            operation_id,
            super::wire::OperationState::Failed,
...
fn qualification_failure_document(operation_id: &str, code: &str, detail: &str) -> ProblemDocument {
    ProblemDocument {
        schema: PROBLEM_SCHEMA.into(),
        r#type: format!("https://sparkplane.local/problems/{code}"),
        code: code.into(),
...
fn qualification_executor_problem(error: ExecutorClientError) -> Response {
    let (status, code, detail) = match error.code {
        sparkplane_core::ErrorCode::BadRequest => (
            StatusCode::BAD_REQUEST,
            "spark.qualification.rejected",
            "executor rejected the signed qualification manifest",
        ),
        sparkplane_core::ErrorCode::Overloaded => (
            StatusCode::CONFLICT,
            "spark.qualification.busy",
            "another qualification job owns the GPU qualification slot",
        ),
        sparkplane_core::ErrorCode::PolicyDenied => (
        ...
        sparkplane_core::ErrorCode::Cancelled => (
            StatusCode::CONFLICT,
            "spark.qualification.cancelled",
            "qualification was cancelled",
        ),
        _ => (
            StatusCode::SERVICE_UNAVAILABLE,
            "spark.qualification.executor-unavailable",
            "qualification executor is unavailable",
        ),
    };
...
#[utoipa::path(get, path = "/api/sparkplane/v1/tokens", responses((status = 200, body = TokenListDocument)))]
...
#[utoipa::path(delete, path = "/api/sparkplane/v1/tokens/{id}", params(("id" = String, Path)), responses((status = 202, body = OperationDocument)))]
...
#[utoipa::path(get, path = "/api/sparkplane/v1/doctor", responses((status = 200, body = DoctorDocument)))]
...
#[utoipa::path(get, path = "/api/sparkplane/v1/certificates/status", responses((status = 200, body = CertificateStatusDocument)))]
...
fn parse_bearer(value: &str) -> Option<(&str, &str)> {
    let rest = value
        ...
        .or_else(|| value.strip_prefix("sy_"))?;
...
fn required_scope(method: &Method, path: &str) -> Option<TokenScope> {
    ...
    if path == format!("{API_BASE}/qualifications") {
...
pub async fn serve(
    ...
    anyhow::ensure!(
        config.model_catalog == Path::new("/etc/sparkplane/models.toml"),
    ...
    let qualification_authority =
        tokio::fs::read_to_string(super::qualification::RELEASE_PUBLIC_KEY_PATH).await?;
    ...
    let database = DbActor::open(
        "/var/lib/sparkplane/state.sqlite3",
        "/var/lib/sparkplane/backups",
    ...
    tokio::spawn(async move {
        ...
        sparkplane_core::notify::stopping();
        shutdown.graceful_shutdown(Some(Duration::from_secs(10)));
    });
    sparkplane_core::notify::ready();
    let _watchdog = sparkplane_core::notify::spawn_watchdog();
    spawn_tls_reload(
        tls.clone(),
...
#[cfg(test)]
mod tests {
    use super::{API_BASE, AgentState, ApiDoc, resolve_download_target, router};
    use axum::{
        body::Body,
        http::{Request, header},
    };
    ...
    fn legacy_recipe_benchmark_and_tuning_paths_are_not_public() {
        ...
        for path in [
            "/api/sparkplane/v1/recipes",
            "/api/sparkplane/v1/benchmarks",
            "/api/sparkplane/v1/tunings",
        ] {
    ...
    async fn metrics_are_authenticated_bounded_and_content_free() {
        ...
        let denied = router(state.clone())
            .oneshot(
                Request::get("/api/sparkplane/v1/metrics")
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
        ...
        assert!(text.contains("sparkplane_agent_up 1"));
    ...
    async fn stopped_intent_interrupts_engine_readiness_wait() {
        ...
        let server = tokio::spawn(sparkplane_ipc::Server::new(ResourceExecutor).serve(listener));
    ...
    async fn exited_container_interrupts_engine_readiness_wait() {
        ...
        let listener = tokio::net::UnixListener::bind(&socket).unwrap();
        let server =
            tokio::spawn(sparkplane_ipc::Server::new(StoppedResourceExecutor).serve(listener));
    ...
    #[test]
    fn qualification_terminal_preserves_output_limit_classification() {
        let failure =
            super::qualification_terminal_failure(crate::spark::executor::ExecutorClientError {
                code: sparkplane_core::ErrorCode::PolicyDenied,
    ...
    #[derive(Clone)]
    struct QualificationExecutor {
        plan: crate::spark::qualification::QualificationPlanDocument,
        executions: Arc<AtomicUsize>,
    }

    impl sparkplane_ipc::Handler for QualificationExecutor {
        async fn handle(&self, request: sparkplane_ipc::Request) -> sparkplane_ipc::Response {
            let action = &request.params["action"];
            let result = if action.get("inspect_qualification").is_some() {
                serde_json::json!({"action":"inspect_qualification","plan":self.plan})
            } else if action.get("run_qualification").is_some() {
                self.executions.fetch_add(1, Ordering::SeqCst);
                let execution = crate::spark::qualification::QualificationExecution::from_output(
                    b"qualification trace\n",
                    b"",
                    0,
                )
                .unwrap();
                serde_json::json!({"action":"run_qualification","plan":self.plan,"execution":execution})
            } else {
                return ResourceExecutor.handle(request).await;
            };
            sparkplane_ipc::Response::Ok {
    ...
    struct CancellableQualificationExecutor {
        plan: crate::spark::qualification::QualificationPlanDocument,
        cancellation: tokio_util::sync::CancellationToken,
        started: Arc<tokio::sync::Notify>,
        cleanup_gate: Arc<tokio::sync::Semaphore>,
        completed: Arc<tokio::sync::Semaphore>,
        cancel_attempts: Arc<AtomicUsize>,
    }

    impl sparkplane_ipc::Handler for CancellableQualificationExecutor {
        async fn handle(&self, request: sparkplane_ipc::Request) -> sparkplane_ipc::Response {
            let action = &request.params["action"];
            if action.get("inspect_qualification").is_some() {
                return sparkplane_ipc::Response::Ok {
                    schema_version: sparkplane_ipc::SCHEMA_VERSION,
                    request_id: request.request_id,
                    result: serde_json::json!({"action":"inspect_qualification","plan":self.plan}),
                    blob: None,
                };
            }
            if action.get("run_qualification").is_some() {
                self.started.notify_one();
                self.cancellation.cancelled().await;
                let _cleanup = self.cleanup_gate.acquire().await.unwrap();
                self.completed.add_permits(1);
                return sparkplane_ipc::Response::Err {
                    schema_version: sparkplane_ipc::SCHEMA_VERSION,
                    request_id: request.request_id,
                    error: sparkplane_ipc::ErrorBody {
                        code: sparkplane_core::ErrorCode::Cancelled,
                        message: "qualification cancelled after cleanup".into(),
                        retry_after_ms: None,
            ...
            if request.method == "system.cancel" {
                if self.cancel_attempts.fetch_add(1, Ordering::SeqCst) == 0 {
                    return sparkplane_ipc::Response::Ok {
                        schema_version: sparkplane_ipc::SCHEMA_VERSION,
                        request_id: request.request_id,
                        result: serde_json::json!({"cancelled":false}),
                        blob: None,
                    };
                }
                self.cancellation.cancel();
                let _completed = self.completed.acquire().await.unwrap();
                return sparkplane_ipc::Response::Ok {
                    schema_version: sparkplane_ipc::SCHEMA_VERSION,
                    request_id: request.request_id,
    ...
    impl sparkplane_ipc::Handler for StoppedResourceExecutor {
        async fn handle(&self, request: sparkplane_ipc::Request) -> sparkplane_ipc::Response {
    ...
    impl sparkplane_ipc::Handler for ResourceExecutor {
        async fn handle(&self, request: sparkplane_ipc::Request) -> sparkplane_ipc::Response {
            let action = &request.params["action"];
            let result = if action == "health" {
                serde_json::json!({"action":"health","health":{"schema":"sparkplane.executor.health/v1","version":"test","authorized_agent_uid":996,"guard_heartbeat":true,"event_heartbeat":true}})
            } else if action == "inspect_protected_host" {
                serde_json::json!({"action":"inspect_protected_host","host":{"schema":"sparkplane.executor.host/v1","hostname":"spark-test","kernel_release":"6.17-test","architecture":"aarch64","identity_sha256":"a".repeat(64)}})
            } else if action == "inspect_docker_version" {
                serde_json::json!({"action":"inspect_docker_version","docker":{"schema":"sparkplane.executor.docker/v1","transport":"unix","version":"29.2.1","api_version":"1.53","minimum_api_version":"1.44","os":"linux","architecture":"aarch64","experimental":false}})
            } else if action == "inspect_resources" {
                serde_json::json!({"action":"inspect_resources","snapshot":{"schema":"sparkplane.resources.snapshot/v1","observed_at_unix_ms":crate::spark::resources::unix_millis(),"mem_total_bytes":127775277056_u64,"mem_available_bytes":107374182400_u64,"memory_full_psi_avg10_percent":0.0,"swap_in_pages_delta":0,"disk_available_bytes":700_u64*1024*1024*1024},"policy":{"system_reserve_bytes":8_u64*1024*1024*1024,"emergency_available_floor_bytes":8_u64*1024*1024*1024,"disk_reserve_bytes":100_u64*1024*1024*1024,"startup_guard_interval_ms":500,"steady_guard_interval_ms":2000,"emergency_consecutive_samples":3,"memory_full_psi_avg10_percent":2.0}})
    ...
    impl sparkplane_ipc::Handler for CountingReconcileExecutor {
        async fn handle(&self, request: sparkplane_ipc::Request) -> sparkplane_ipc::Response {
            let action = &request.params["action"];
    ...
    impl sparkplane_ipc::Handler for StopRaceExecutor {
        async fn handle(&self, request: sparkplane_ipc::Request) -> sparkplane_ipc::Response {
            let action = &request.params["action"];
            let result = if let Some(expected) = action.get("reconcile_scan") {
                ...
                if self.fail_while_stopping && attempt == 0 {
                    return sparkplane_ipc::Response::Err {
                        ...
                        request_id: request.request_id,
                        error: sparkplane_ipc::ErrorBody {
                            code: sparkplane_core::ErrorCode::Internal,
    ...
    impl sparkplane_ipc::Handler for MatchedReconcileExecutor {
        async fn handle(&self, request: sparkplane_ipc::Request) -> sparkplane_ipc::Response {
            let action = &request.params["action"];
            let result = if let Some(expected) = action.get("reconcile_scan") {
                let identity = &expected[0];
    ...
    fn test_artifacts() -> crate::spark::wire::ModelArtifactsDocument {
        serde_json::from_str(r#"{"schema":"sparkplane.model-artifacts/v2","format":"safetensors","primary":{"path":"model.safetensors","bytes":8,"sha256":null},"auxiliary":[],"quantization":"FP8","capabilities":["text_generation"],"configured_alias":null}"#).unwrap()
    }
    ...
    #[test]
    fn published_route_requires_the_same_live_process() {
        ...
        assert!(super::exact_route_is_published(
            &routes,
            &instance,
            (123, 456)
        ));
    ...
    async fn reconcile_restarts_the_same_engine_and_artifact_generation() {
        ...
        let server = tokio::spawn(
            sparkplane_ipc::Server::new(CountingReconcileExecutor {
    ...
    async fn matched_event_cannot_complete_an_active_semantic_probe() {
        ...
        let server = tokio::spawn(
            sparkplane_ipc::Server::new(MatchedReconcileExecutor {
    ...
    async fn stopped_intent_recovers_over_real_executor_protocol_without_model_removal() {
        ...
        let listener = tokio::net::UnixListener::bind(&socket).unwrap();
        let server = tokio::spawn(async move {
            let _ = sparkplane_ipc::Server::new(ResourceExecutor)
                .serve(listener)
                .await;
        });
    ...
    async fn explicit_stop_completes_one_removal_while_transition_is_busy() {
        ...
        let server = tokio::spawn(
            sparkplane_ipc::Server::new(StopRaceExecutor {
    ...
    fn signed_qualification(
        operation: crate::spark::qualification::QualificationOperation,
    ) -> (
        String,
        crate::spark::qualification::QualificationSubmission,
        crate::spark::qualification::QualificationPlanDocument,
        ...
        use std::io::Cursor;
        let target = crate::spark::qualification::QualificationTarget {
            architecture: "aarch64".into(),
            gpu_model: "NVIDIA GB10".into(),
            compute_capability: "12.1".into(),
            dgx_build: "7.5.0".into(),
            driver_version: "580.159.03".into(),
            toolkit_version: "1.19.0".into(),
            protected_fingerprint: format!("sha256:{}", "7".repeat(64)),
        };
        let manifest = crate::spark::qualification::QualificationManifest {
            schema: crate::spark::qualification::QUALIFICATION_MANIFEST_SCHEMA.into(),
            job_id: "spark-flash-adaptive-decoding".into(),
            operation,
            image: format!("registry.example/sparky@sha256:{}", "1".repeat(64)),
            target: target.clone(),
            limits: crate::spark::qualification::QualificationLimits {
        ...
        let signature = minisign::sign(None, &keys.sk, Cursor::new(&bytes), None, None)
            .unwrap()
            .into_string();
        let submission = crate::spark::qualification::QualificationSubmission {
            manifest_base64: base64::engine::general_purpose::STANDARD.encode(bytes),
            signature,
            dry_run: true,
        };
        let mut plan = crate::spark::qualification::verify_submission(
            &submission,
            &keys.pk.to_base64(),
            &target,
        )
        ...
        .plan;
        plan.resources = Some(
            crate::spark::qualification::QualificationResourceAdmission {
                observed_at_unix_ms: crate::spark::resources::unix_millis(),
                mem_available_bytes: 32 * 1024 * 1024 * 1024,
                live_available_after_start_bytes: 24 * 1024 * 1024 * 1024,
                required_available_floor_bytes: 8 * 1024 * 1024 * 1024,
                disk_available_bytes: 200 * 1024 * 1024 * 1024,
                disk_available_after_pull_bytes: 184 * 1024 * 1024 * 1024,
                required_disk_reserve_bytes: 100 * 1024 * 1024 * 1024,
                memory_full_psi_avg10_percent: 0.0,
                swap_in_pages_delta: 0,
            },
        );
        (keys.pk.to_base64(), submission, plan)
    }

    #[tokio::test]
    async fn qualification_dry_run_and_yes_are_daemon_e2e_with_fake_runtime() {
        use crate::spark::qualification::QualificationOperation as Op;
        for kind in [
            Op::SparkFlashAdaptiveDecodingV1,
            Op::SparkFlashQsaV1,
            Op::SparkFlashMoeV1,
            Op::SparkFlashKernelCatalogV1,
            Op::SparkFlashCudaLifecycleV1,
            Op::SparkFlashMemoryAdmissionV1,
            Op::SparkFlashGraphCatalogV1,
            Op::SparkFlashSessionV1,
            Op::SparkFlashModelPrefixV1,
        ] {
            let (state, database, root) = durable_state().await;
            let (authority, submission, plan) = signed_qualification(kind);
            let socket = root.join("qualification-executor.sock");
            let listener = tokio::net::UnixListener::bind(&socket).unwrap();
            let executions = Arc::new(AtomicUsize::new(0));
            let server = tokio::spawn(
                sparkplane_ipc::Server::new(QualificationExecutor {
                    plan,
            ...
            let state = state
                .with_executor(crate::spark::executor::ExecutorClient::new(socket))
                .with_qualification_authority(Arc::<str>::from(authority));
            let request = |submission: &crate::spark::qualification::QualificationSubmission,
                           key: &str| {
                Request::post("/api/sparkplane/v1/qualifications")
                    .header(header::AUTHORIZATION, format!("Bearer {TOKEN}"))
            ...
            let dry = router(state.clone())
                .oneshot(request(&submission, "qualification-dry-run"))
                .await
                .unwrap();
            assert_eq!(dry.status(), axum::http::StatusCode::OK);
            assert_eq!(executions.load(Ordering::SeqCst), 0);
            assert!(database.list_operations().await.unwrap().is_empty());

            let mut execute = submission;
            execute.dry_run = false;
            let accepted = router(state)
                .oneshot(request(&execute, "qualification-execute"))
                .await
                .unwrap();
            ...
            let operation = database.list_operations().await.unwrap().remove(0);
            assert_eq!(
                operation.state,
                crate::spark::wire::OperationState::Succeeded
            );
            assert_eq!(executions.load(Ordering::SeqCst), 1);
            let result: crate::spark::qualification::QualificationResultDocument =
                serde_json::from_value(operation.result.unwrap()).unwrap();
            assert_eq!(result.plan.operation, kind);
            assert_eq!(
                result.execution.decode_stdout().unwrap(),
                b"qualification trace\n"
            );
            database.shutdown().unwrap();
            server.abort();
    ...
    #[tokio::test]
    async fn qualification_cancel_is_owner_checked_and_terminal_only_after_cleanup() {
        let (state, database, root) = durable_state().await;
        let (authority, mut submission, plan) = signed_qualification(
            crate::spark::qualification::QualificationOperation::SparkFlashAdaptiveDecodingV1,
        );
        submission.dry_run = false;
        let socket = root.join("cancellable-qualification-executor.sock");
        let listener = tokio::net::UnixListener::bind(&socket).unwrap();
        let cancellation = tokio_util::sync::CancellationToken::new();
        let started = Arc::new(tokio::sync::Notify::new());
        let cleanup_gate = Arc::new(tokio::sync::Semaphore::new(0));
        let cancel_attempts = Arc::new(AtomicUsize::new(0));
        let server = tokio::spawn(
            sparkplane_ipc::Server::new(CancellableQualificationExecutor {
                plan,
        ...
        let accepted = router(state.clone())
            .oneshot(
                Request::post("/api/sparkplane/v1/qualifications")
                    .header(header::AUTHORIZATION, format!("Bearer {TOKEN}"))
                    .header("Idempotency-Key", "qualification-cancel")
                    .header(header::CONTENT_TYPE, "application/json")
                    .body(Body::from(serde_json::to_vec(&submission).unwrap()))
                    .unwrap(),
            )
            .await
            .unwrap();
        let operation: crate::spark::wire::OperationDocument = serde_json::from_slice(
            &axum::body::to_bytes(accepted.into_body(), 65_536)
                .await
                .unwrap(),
        )
        .unwrap();
        ...
        let outsider = database
            .create_token(
                "bootstrap-admin",
                "qualification-cancel-outsider",
                crate::spark::wire::TokenCreateRequest {
                    name: "qualification-cancel-outsider".into(),
                    scopes: vec![crate::spark::wire::TokenScope::OperationsCancel],
        ...
        let delete = |bearer: &str, key: &str| {
            Request::delete(format!("/api/sparkplane/v1/operations/{}", operation.id))
        ...
        let denied = router(state.clone())
            .oneshot(delete(
                &outsider.bearer_token.unwrap(),
                "outsider-cancel-attempt",
            ))
            .await
            .unwrap();
        ...
        let cancel = tokio::spawn(router(state).oneshot(delete(TOKEN, "owner-cancel")));
        ...
        assert_eq!(
            cancel.await.unwrap().unwrap().status(),
            axum::http::StatusCode::ACCEPTED
        );
        assert_eq!(
            database.operation(&operation.id).await.unwrap().state,
            crate::spark::wire::OperationState::Cancelled
        );
        assert_eq!(cancel_attempts.load(Ordering::SeqCst), 2);
        database.shutdown().unwrap();
        server.abort();
    }
            ...
            .promote_model(
                crate::spark::wire::ModelDocument {
                    ...
                    artifacts: Some(crate::spark::wire::ModelArtifactsDocument {
                        schema: "sparkplane.model-artifacts/v2".into(),
    ...
    async fn authenticated_status_is_degraded_without_executor() {
        ...
        let status: crate::spark::wire::StatusDocument = client
            .get(format!(
                "https://localhost:{}/api/sparkplane/v1/status",
                address.port()
            ))
            .bearer_auth(TOKEN)
            .send()
    ...
    #[test]
    fn qualification_openapi_declares_the_closed_model_prefix_wire_value() {
        let document = serde_json::to_value(super::ApiDoc::openapi()).unwrap();
        let values = document["components"]["schemas"]["QualificationOperation"]["enum"]
            .as_array()
            .unwrap();
        assert!(values.contains(&serde_json::json!("spark_flash_model_prefix_v1")));
    }
    ...
    fn generated_openapi_matches_the_normalized_fixture() {
        ...
        let fixture: serde_json::Value = serde_json::from_str(include_str!(
            "../../specs/openapi/sparkplane-control-v1.json"
        ))
        .unwrap();
