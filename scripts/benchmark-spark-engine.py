...
def validate_plan(plan: dict) -> None:
    ...
    if set(plan) != required or plan["schema"] != "sparkplane.benchmark-plan/v1":
...
def validate_observations(observations: dict) -> None:
    ...
    if set(observations) != {"schema", *categories} or observations.get("schema") != "sparkplane.benchmark-observations/v1":
...
def validate_result(result: dict) -> None:
    ...
    if result.get("schema") != "sparkplane.benchmark-result/v1" or result.get("mode") != "live":
...
def compare_results(first: dict, second: dict) -> dict:
    ...
    return {"schema": "sparkplane.benchmark-comparison/v1", "plan_sha256": first["plan_sha256"], "model_fingerprint": first["model_fingerprint"], "first_engine_fingerprint": first["engine_fingerprint"], "second_engine_fingerprint": second["engine_fingerprint"], "workloads": workloads}
...
def main() -> None:
    ...
    if args.dry_run:
        result = {"schema": "sparkplane.benchmark-result/v1", "mode": "dry-run", "plan_sha256": plan_hash, "sampling_sha256": document_sha256(plan["sampling"]), "planned_requests": planned, "workload_ids": [workload["id"] for workload in plan["workloads"]]}
        print(json.dumps(result, sort_keys=True, separators=(",", ":")))
        return
