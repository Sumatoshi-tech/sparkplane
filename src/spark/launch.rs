...
const LAUNCH_STATE_SCHEMA: &str = "sparkplane.launch-state/v1";
const LAUNCH_PLAN_SCHEMA: &str = "sparkplane.launch-plan/v1";
const OWNED_MARKER: &str = "owned-by: sparkplane launch";
const INFERENCE_CONCURRENCY: u32 = 8;
const CODEX_PROFILE: &str = "sparkplane-launch";
...
pub(crate) fn migrate_legacy_state(text: &str) -> Result<String, String> {
    let mut state: LaunchState = toml::from_str(text).map_err(|error| error.to_string())?;
    if state.schema != "sy.spark.launch-state/v1" {
        return Err("unsupported legacy launch-state schema".into());
    }
    state.schema = LAUNCH_STATE_SCHEMA.into();
    toml::to_string(&state).map_err(|error| error.to_string())
}
...
fn choose_model_reference(
    ...
    if let Some(value) = saved
        && models.models.iter().any(|model| model.id == value.model_id)
    {
...
fn select_model(models: &ModelListDocument) -> Result<String, ClientError> {
    if models.models.is_empty() {
        return Err(usage(
            "no verified Spark models are installed; run `sparkplane <host> download <repository> --revision <commit> --alias <name>`",
        ));
    }
...
fn resolve_model(models: &[ModelDocument], reference: &str) -> Result<ModelDocument, ClientError> {
    ...
    match matches.as_slice() {
        ...
        [] => Err(usage(format!(
            "Spark model {reference:?} is not installed; run `sparkplane <host> download <repository> --revision <commit> --alias {reference}`"
        ))),
...
fn ensure_inference_token(
    ...
    let created = client.create_token(
        &uuid::Uuid::new_v4().to_string(),
        &TokenCreateRequest {
            name: format!("sparkplane-launch@{local_name}"),
...
fn usable_launch_token(token: &TokenDocument, id: &str) -> bool {
    token.id == id
        ...
        && token.name.starts_with("sparkplane-launch@")
}

fn owned_launch_token(token: &TokenDocument, id: &str) -> bool {
    token.id == id && token.name.starts_with("sparkplane-launch@")
}
...
fn configure_codex_command(
    ...
    let mut overrides = vec![
        ...
        format!(
            ...
            format!("Sparkplane {}", instance.name)
        ),
        format!(
...
fn configure_opencode_command(
    ...
    let content = serde_json::json!({
        ...
        "provider": {
            "sparkplane": {
                ...
                "options": {
                    ...
                    "apiKey": "{env:SPARKPLANE_INFERENCE_TOKEN}"
                },
                "models": {
        ...
        "model": format!("sparkplane/{}", model.canonical)
    });
    command
        .args(extra_args)
        .env(
...
fn provider_name(instance: &str) -> String {
    format!("sparkplane_{}", instance.replace(['-', '.'], "_"))
}
    ...
    for (index, arg) in args.iter().enumerate() {
        ...
        if conflicts {
            return Err(usage(format!(
                "conflicting Codex agent argument {arg:?}: sparkplane launch manages model and provider routing"
            )));
        }
    }
    Ok(())
...
fn acquire_state_lock(config_dir: &Path) -> Result<nix::fcntl::Flock<fs::File>, ClientError> {
    ...
    Flock::lock(file, FlockArg::LockExclusiveNonblock).map_err(|_| {
        failure(
            EXIT_REJECTED,
            "another sparkplane launch is updating local launch state",
        )
    })
}
...
mod tests {
    ...
    fn artifacts() -> crate::spark::wire::ModelArtifactsDocument {
        serde_json::from_str(r#"{"schema":"sparkplane.model-artifacts/v2","format":"gguf","primary":{"path":"model.gguf","bytes":8,"sha256":null},"auxiliary":[],"quantization":"Q4_K_XL","capabilities":["text_generation"],"configured_alias":null}"#).unwrap()
    }
    ...
    fn opencode_adapter_uses_inline_provider_without_embedding_secret() {
        ...
        let content = command_env(&command, "OPENCODE_CONFIG_CONTENT")
            ...
            .unwrap();
        let config: Value = serde_json::from_str(&content).unwrap();
        assert_eq!(config["model"], format!("sparkplane/{}", model.canonical));
        assert_eq!(
            config["provider"]["sparkplane"]["options"]["baseURL"],
            "https://127.0.0.1:9843/openai/ornith/v1"
        );
        assert_eq!(
            config["provider"]["sparkplane"]["options"]["apiKey"],
            "{env:SPARKPLANE_INFERENCE_TOKEN}"
        );
    ...
    fn only_exact_inference_token_is_usable() {
        let token = TokenDocument {
            schema: "sparkplane.token/v1".into(),
            id: "01ABCDEFGHIJKLMNOPQRSTUVWX".into(),
            name: "sparkplane-launch@host".into(),
            scopes: vec![TokenScope::Inference],
            allowed_cidrs: Vec::new(),
            expires_at: None,
            max_concurrent_inference: INFERENCE_CONCURRENCY,
