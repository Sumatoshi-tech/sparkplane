...
def spdx(packages, image_digest, created):
    ...
    document = {
        ...
        "creationInfo": {
            "created": created,
            "creators": ["Tool: sparkplane-image-inventory/1"],
        },
        "dataLicense": "CC0-1.0",
        "documentNamespace": (
            f"https://sparkplane.local/spdx/{image_digest.removeprefix('sha256:')}/"
            f"{hashlib.sha256(frozen.encode()).hexdigest()}"
        ),
        "name": f"sparkplane-image-{image_digest.removeprefix('sha256:')}",
        "packages": [],
        "relationships": [],
