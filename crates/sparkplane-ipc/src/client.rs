...
use crate::envelope::{Request, Response, SCHEMA_VERSION, SpanId, TraceId};
use crate::stream::EventCodec;
use sparkplane_core::Priority;
...
impl Client {
    ...
    pub async fn call(
        ...
        sparkplane_core::obs::with_trace_id_async(trace_id, parent, async move {
...
fn build_request(method: &str, params: serde_json::Value, opts: CallOpts) -> Request {
    ...
    let trace_id = Some(opts.trace_id.unwrap_or_else(|| {
        sparkplane_core::obs::current_trace_ctx()
            .map(|tc| tc.trace_id)
            .unwrap_or_default()
    }));
    Request {
...
mod tests {
    ...
    fn call_inherits_ambient_trace_id_from_with_trace_id() {
        ...
        let subscriber = Registry::default().with(sparkplane_core::obs::TraceCtxLayer::new());
        let dispatch = tracing::Dispatch::new(subscriber);
        let observed = std::sync::Arc::new(std::sync::Mutex::new(None::<TraceId>));
        let observed_clone = observed.clone();
        tracing::dispatcher::with_default(&dispatch, || {
            sparkplane_core::obs::with_trace_id(TraceId(T.into()), None, || {
