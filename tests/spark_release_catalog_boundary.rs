...
fn production_installer_does_not_embed_operational_catalogs() {
    ...
    assert!(!compact.contains("include_bytes!(\"../../configs/sparkplane/engines/"));
    assert!(!compact.contains("include_str!(\"../../configs/sparkplane/engines/"));
    ...
    for name in model_catalog_names() {
        assert!(!compact.contains(&format!(
            "include_bytes!(\"../../configs/sparkplane/{name}\")"
        )));
        assert!(!compact.contains(&format!(
            "include_str!(\"../../configs/sparkplane/{name}\")"
        )));
    }
}
...
fn engine_images_record_frozen_provenance() {
    for dockerfile in engine_dockerfiles() {
        ...
        assert!(dockerfile.contains("sparkplane.base-manifest=\"sha256:"));
        assert!(dockerfile.contains("sparkplane.cuda-architecture="));
    }
}
...
fn all_engine_tomls_are_signed_without_family_names() {
    let fixture = tempfile::tempdir().unwrap();
    let binary = fixture.path().join("sparkplane-aarch64");
    let release = fixture.path().join("release");
    fs::write(&binary, b"arm64 fixture").unwrap();
    assert!(
        Command::new("scripts/package-spark-release.sh")
            .args([&binary, &release])
            .status()
            .unwrap()
            .success()
    ...
    let signed = manifest
        ...
        .filter(|path| path.starts_with("configs/sparkplane/engines/"))
        .collect::<BTreeSet<_>>();
    let expected = engine_config_names()
        .into_iter()
        .map(|name| format!("configs/sparkplane/engines/{name}"))
        .collect::<BTreeSet<_>>();
...
fn engine_config_names() -> Vec<String> {
    let directory = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("configs/sparkplane/engines");
...
fn model_catalog_names() -> Vec<String> {
    let directory = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("configs/sparkplane");
    fs::read_dir(directory)
        .unwrap()
        .filter_map(Result::ok)
        .filter(|entry| entry.path().extension().is_some_and(|ext| ext == "toml"))
...
fn configured_model_identifiers() -> BTreeSet<String> {
    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("configs/sparkplane/models.toml");
...
fn contracted_engine_policies() -> Vec<toml::Value> {
    ...
    fs::read_dir(directory)
        ...
        .filter(|entry| {
            fs::read_to_string(entry.path())
                .is_ok_and(|source| source.contains("sparkplane.image-contract=\"v1\""))
        })
...
fn engine_dockerfiles() -> Vec<String> {
    ...
    fs::read_dir(directory)
        ...
        .map(|entry| fs::read_to_string(entry.path()).unwrap())
        .filter(|dockerfile| dockerfile.contains("sparkplane.image-contract=\"v1\""))
        .collect()
}
