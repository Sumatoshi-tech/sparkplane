...
use super::upstream::{
    DEFAULT_STREAM_IDLE_TIMEOUT, GenerationEvent, ImagePart, ObservedRoute,
...
pub(crate) fn rewrite_completion_request(
    bytes: &[u8],
    served_model: &str,
) -> Result<(Vec<u8>, bool), ()> {
...
pub(crate) fn rewrite_completion_response(bytes: &[u8], public_model: &str) -> Result<Value, ()> {
    let mut value: Value = serde_json::from_slice(bytes).map_err(|_| ())?;
    value
        .as_object_mut()
        .ok_or(())?
