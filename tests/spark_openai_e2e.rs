...
fn responses_stateless_function_and_custom_tool_continuation_is_protocol_native() {
    ...
    assert!(
        gateway::rewrite_responses_request(
...
#[test]
#[ignore = "set SPARKPLANE_CODEX_BIN to the pinned Codex 0.149.0 binary"]
fn pinned_codex_0_149_completes_one_streamed_client_tool_round_trip() {
    let binary = std::env::var("SPARKPLANE_CODEX_BIN").expect("exact Codex binary path");
    ...
    let output = Command::new(binary)
        ...
        .env("SPARKPLANE_INFERENCE_TOKEN", "fixture-only-token")
