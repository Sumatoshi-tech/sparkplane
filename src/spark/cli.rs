...
#[derive(Debug, Args)]
pub struct ClientMigrationArgs {
    #[arg(long, env = "SPARKPLANE_LEGACY_CONFIG_DIR")]
...
fn default_probe(env: &dyn EnvSource) -> PathBuf {
    ...
    data.join("sparkplane/appliance-release/sparkplane-aarch64")
}
...
pub enum SparkCommand {
    ...
    #[command(
        after_help = "Examples:\n  sparkplane dgx-spark install --dry-run --json\n  sparkplane dgx-spark install --yes --release-manifest SHA256SUMS --release-signature SHA256SUMS.minisig --release-public-key sparkplane-release.pub\n\nEnvironment:\n  SPARKPLANE_DRY_RUN, SPARKPLANE_YES, SPARKPLANE_JSON, SPARKPLANE_PROBE, SPARKPLANE_RELEASE_MANIFEST, SPARKPLANE_LISTEN_ADDRESS, SPARKPLANE_LISTEN_PORT, SPARKPLANE_RELEASE_SIGNATURE, SPARKPLANE_RELEASE_PUBLIC_KEY, SPARKPLANE_CONFIG_DIR\n  Flags override environment values, which override declarative defaults.\n\nAuthentication:\n  OpenSSH owns known_hosts, agents, hardware tokens, keyboard-interactive and interactive password prompts. Credentials are never accepted as Sparkplane arguments or stored by Sparkplane.\n\nExit codes:\n  0 success; 2 usage/local configuration; 4 OpenSSH/SFTP/agent unreachable, TLS identity mismatch, or authentication failure.\n\nSecurity:\n  Dry-run uploads one content-addressed probe, invokes only `sparkplane bootstrap inspect`, verifies its hash, and removes that exact path. Approved install uploads only signed content-addressed release inputs and invokes the fixed bootstrap activation entrypoint. No arbitrary remote command is accepted."
    )]
    Install(InstallArgs),
    /// Upgrade the signed control plane side by side without stopping engines.
    #[command(
        after_help = "Examples:\n  sparkplane dgx-spark upgrade --dry-run --json\n  sparkplane dgx-spark upgrade --yes --release-manifest SHA256SUMS --release-signature SHA256SUMS.minisig --release-public-key sparkplane-release.pub --json\n\nEnvironment:\n  SPARKPLANE_DRY_RUN, SPARKPLANE_YES, SPARKPLANE_JSON, SPARKPLANE_PROBE, SPARKPLANE_RELEASE_MANIFEST, SPARKPLANE_RELEASE_SIGNATURE, SPARKPLANE_RELEASE_PUBLIC_KEY, SPARKPLANE_CONFIG_DIR\n\nExit codes:\n  0 success; 2 usage/local configuration; 3 compatibility or safety rejection; 4 SSH, TLS, or agent unreachable."
    )]
    Upgrade(InstallArgs),
    /// Atomically return to the verified preceding control-plane release.
    #[command(
        after_help = "Examples:\n  sparkplane dgx-spark rollback --dry-run --json\n  sparkplane dgx-spark rollback --yes --json\n\nHealthy engine containers are preserved. Docker restart and host reboot are never performed.\n\nEnvironment:\n  SPARKPLANE_DRY_RUN, SPARKPLANE_YES, SPARKPLANE_JSON, SPARKPLANE_CONFIG_DIR"
    )]
    ...
    Doctor(ReadArgs),
    /// Preview or run one release-authority-signed GPU qualification job.
    #[command(
        after_help = "Examples:\n  sparkplane dgx-spark qualify --manifest sparky-adaptive.json --signature sparky-adaptive.json.minisig --dry-run --json\n  sparkplane dgx-spark qualify --manifest sparky-adaptive.json --signature sparky-adaptive.json.minisig --yes --json\n\nThe signed manifest selects a finite built-in operation and digest-pinned OCI bundle. There is no shell, executable, argv, mount, network, privilege, or resource-override flag.\n\nEnvironment:\n  SPARKPLANE_QUALIFICATION_MANIFEST, SPARKPLANE_QUALIFICATION_SIGNATURE, SPARKPLANE_DRY_RUN, SPARKPLANE_YES, SPARKPLANE_DETACH, SPARKPLANE_JSON, SPARKPLANE_IDEMPOTENCY_KEY, SPARKPLANE_CONFIG_DIR"
    )]
    Qualify(QualificationArgs),
    /// Inspect, follow, or cancel durable operations.
    #[command(
        after_help = "Examples:\n  sparkplane dgx-spark operations --json\n  sparkplane dgx-spark operations 01K... --follow --json\n  sparkplane dgx-spark operations cancel 01K... --dry-run\n\nEnvironment:\n  SPARKPLANE_JSON, SPARKPLANE_CONFIG_DIR, SPARKPLANE_IDEMPOTENCY_KEY, SPARKPLANE_DRY_RUN"
    )]
    Operations(OperationsArgs),
    /// Create, list, or revoke scoped bearer tokens.
    #[command(
        after_help = "Examples:\n  sparkplane dgx-spark token create --name reader --scope models:read --scope operations:read --detach --json\n  sparkplane dgx-spark token list --json\n  sparkplane dgx-spark token revoke 01K... --yes --json\n\nSecrets are returned once by create and are never written by Sparkplane outside stdout.\n\nEnvironment:\n  SPARKPLANE_JSON, SPARKPLANE_CONFIG_DIR, SPARKPLANE_IDEMPOTENCY_KEY, SPARKPLANE_DRY_RUN, SPARKPLANE_YES"
    )]
    Token(TokenArgs),
    /// Acquire and verify one immutable Hugging Face model snapshot.
    #[command(
        after_help = "Examples:\n  sparkplane dgx-spark download ornith-1.5:35b --dry-run --json\n  sparkplane dgx-spark download ornith-1.5:35b\n  sparkplane dgx-spark download owner/model --revision <commit> --artifact model.gguf --auxiliary projector=mmproj.gguf --alias model:q4\n\nAuxiliaries require ROLE=PATH. ROLE is a lowercase identifier that the selected engine configuration must bind or explicitly ignore.\n\nEnvironment:\n  SPARKPLANE_REVISION, SPARKPLANE_ALIAS, SPARKPLANE_ARTIFACT, SPARKPLANE_AUXILIARY, SPARKPLANE_UPDATE_ALIAS, SPARKPLANE_DETACH, SPARKPLANE_DRY_RUN, SPARKPLANE_JSON, SPARKPLANE_CONFIG_DIR"
    )]
    Download(DownloadArgs),
    /// Start one managed model with the configured inference engine.
    #[command(
        after_help = "Examples:\n  sparkplane dgx-spark serve ornith-1.5:35b\n  sparkplane dgx-spark serve ornith-1.5:35b --dry-run --json\n\nServe starts the configured digest-pinned engine for any verified compatible model. The dry-run performs admission without Docker or GPU side effects.\n\nEnvironment:\n  SPARKPLANE_INSTANCE_NAME, SPARKPLANE_DETACH, SPARKPLANE_DRY_RUN, SPARKPLANE_JSON, SPARKPLANE_IDEMPOTENCY_KEY, SPARKPLANE_CONFIG_DIR\n\nExit codes:\n  0 success; 1 unexpected failure; 2 usage/local configuration; 3 remote policy/state rejection; 4 unreachable, TLS pin mismatch, or authentication failure."
    )]
    Serve(ServeArgs),
    ...
    #[command(
        after_help = "Examples:\n  sparkplane dgx-spark logs ornith --limit 100\n  sparkplane dgx-spark logs ornith --json\n\nEnvironment:\n  SPARKPLANE_LOG_CURSOR, SPARKPLANE_LOG_LIMIT, SPARKPLANE_FOLLOW, SPARKPLANE_JSON, SPARKPLANE_CONFIG_DIR"
    )]
    Logs(LogsArgs),
    /// Persist stopped intent, drain, and remove one managed instance.
    #[command(
        after_help = "Examples:\n  sparkplane dgx-spark stop ornith --dry-run --json\n  sparkplane dgx-spark stop ornith\n\nEnvironment:\n  SPARKPLANE_STOP_TIMEOUT_SECONDS, SPARKPLANE_DRY_RUN, SPARKPLANE_JSON, SPARKPLANE_IDEMPOTENCY_KEY, SPARKPLANE_CONFIG_DIR"
    )]
    Stop(StopArgs),
    /// List verified local models available to run.
    #[command(
        after_help = "Examples:\n  sparkplane dgx-spark ls\n  sparkplane dgx-spark ls --json\n\nThe default output is a compact table. Use `show` or `--json` for complete identity and provenance.\n\nEnvironment:\n  SPARKPLANE_JSON, SPARKPLANE_CONFIG_DIR"
    )]
    Ls(ReadArgs),
    /// Show immutable identity, provenance, aliases, and references for one model.
    #[command(
        after_help = "Examples:\n  sparkplane dgx-spark show ornith-1.5:35b\n  sparkplane dgx-spark show ornith-1.5:35b --json\n\nEnvironment:\n  SPARKPLANE_JSON, SPARKPLANE_CONFIG_DIR"
    )]
    Show(ModelReadArgs),
    /// Preview or remove only unreferenced native-cache model data.
    #[command(
        after_help = "Examples:\n  sparkplane dgx-spark rm ornith-1.5:35b --dry-run --json\n  sparkplane dgx-spark rm ornith-1.5:35b --yes\n\nEnvironment:\n  SPARKPLANE_YES, SPARKPLANE_DRY_RUN, SPARKPLANE_JSON, SPARKPLANE_IDEMPOTENCY_KEY, SPARKPLANE_CONFIG_DIR"
    )]
    Rm(RemoveArgs),
...
#[derive(Debug, Args)]
pub struct QualificationArgs {
    /// Exact JSON manifest signed by the installed Spark release authority.
    #[arg(long, env = "SPARKPLANE_QUALIFICATION_MANIFEST")]
    pub manifest: PathBuf,
    /// Minisign signature over the exact manifest bytes.
    #[arg(long, env = "SPARKPLANE_QUALIFICATION_SIGNATURE")]
    pub signature: PathBuf,
    #[arg(long, env = "SPARKPLANE_DRY_RUN")]
    pub dry_run: bool,
    #[arg(long, env = "SPARKPLANE_YES")]
    pub yes: bool,
    #[arg(long, env = "SPARKPLANE_DETACH")]
    pub detach: bool,
    #[arg(long, env = "SPARKPLANE_JSON")]
    pub json: bool,
    #[arg(long, env = "SPARKPLANE_IDEMPOTENCY_KEY")]
...
pub struct DownloadArgs {
    ...
    #[arg(long, default_value = "main", env = "SPARKPLANE_REVISION")]
    pub revision: String,
    #[arg(long, env = "SPARKPLANE_ALIAS")]
    pub alias: Option<String>,
    /// Override automatic artifact selection with an exact primary path.
    #[arg(long, env = "SPARKPLANE_ARTIFACT")]
    pub artifact: Option<String>,
    /// Exact ROLE=PATH auxiliary; repeat for every engine-bound artifact.
    #[arg(long, env = "SPARKPLANE_AUXILIARY", value_delimiter = ',')]
    pub auxiliary: Vec<ModelArtifactSelectorDocument>,
    #[arg(long, env = "SPARKPLANE_UPDATE_ALIAS")]
...
pub struct ServeArgs {
    pub model: String,
    #[arg(long, env = "SPARKPLANE_INSTANCE_NAME")]
...
pub struct LaunchArgs {
    ...
    pub configure: bool,
    #[arg(long, env = "SPARKPLANE_LAUNCH_RESTORE")]
    pub restore: bool,
    #[arg(long, short = 'y', env = "SPARKPLANE_YES")]
    pub yes: bool,
    #[arg(long, env = "SPARKPLANE_DRY_RUN")]
    pub dry_run: bool,
    #[arg(long, env = "SPARKPLANE_JSON")]
...
pub struct StopArgs {
    pub instance: String,
    #[arg(long, default_value_t = 30, env = "SPARKPLANE_STOP_TIMEOUT_SECONDS")]
...
pub struct LogsArgs {
    pub instance: String,
    #[arg(long, env = "SPARKPLANE_LOG_CURSOR", default_value_t = 0)]
    pub cursor: u64,
    #[arg(long, env = "SPARKPLANE_LOG_LIMIT", default_value_t = 100)]
    pub limit: usize,
    #[arg(long, env = "SPARKPLANE_FOLLOW")]
    pub follow: bool,
    #[arg(long, env = "SPARKPLANE_JSON")]
    pub json: bool,
    #[arg(long, env = "SPARKPLANE_CONFIG_DIR")]
...
pub struct AgentArgs {
    #[arg(long, default_value = "/etc/sparkplane/agent.toml")]
    pub config: PathBuf,
    #[arg(long, default_value = "/var/lib/sparkplane/tls/server-chain.pem")]
    pub certificate: PathBuf,
    #[arg(long, default_value = "/var/lib/sparkplane/tls/server-key.pem")]
    pub key: PathBuf,
    #[arg(
        long,
        default_value = "/run/credentials/sparkplane-agent.service/bootstrap-token"
    )]
    pub token: PathBuf,
    #[arg(
        long,
        default_value = "/run/credentials/sparkplane-agent.service/hf-token"
    )]
...
pub struct ExecutorArgs {
    #[arg(long, default_value = "/etc/sparkplane/executor.toml")]
    pub config: PathBuf,
}
...
fn resolve_install(args: &InstallArgs, env: &dyn EnvSource) -> Result<ResolvedInstall, SparkError> {
    let dry_run = args.dry_run || env_bool(env, "SPARKPLANE_DRY_RUN")?.unwrap_or(false);
    let yes = args.yes || env_bool(env, "SPARKPLANE_YES")?.unwrap_or(false);
    if !dry_run && !yes {
    ...
    let json = args.json || env_bool(env, "SPARKPLANE_JSON")?.unwrap_or(false);
    let probe = args
        .probe
        .clone()
        .or_else(|| env.get("SPARKPLANE_PROBE").map(PathBuf::from))
        .unwrap_or_else(|| default_probe(env));
    let release_manifest = args
        .release_manifest
        .clone()
        .or_else(|| env.get("SPARKPLANE_RELEASE_MANIFEST").map(PathBuf::from))
        .unwrap_or_else(|| probe.with_file_name("SHA256SUMS"));
    let listen_address = args
        .listen_address
        .clone()
        .or_else(|| env.get("SPARKPLANE_LISTEN_ADDRESS"));
    let listen_port = match args
        .listen_port
        .map(|port| port.to_string())
        .or_else(|| env.get("SPARKPLANE_LISTEN_PORT"))
    {
        Some(value) => value.parse::<u16>().map_err(|_| {
            SparkError::usage(format!(
                "SPARKPLANE_LISTEN_PORT is not a valid port: {value}"
            ))
        })?,
...
pub fn dispatch(cli: SparkCli) -> anyhow::Result<()> {
    match cli.command {
        SparkCommand::MigrateClient(args) => dispatch_client_migration(&cli.host, args),
        ...
        SparkCommand::Status(args) => dispatch_read::<StatusDocument>(
            &cli.host,
            args,
            "api/sparkplane/v1/status",
            render_status,
        ),
        SparkCommand::Doctor(args) => dispatch_read::<DoctorDocument>(
            &cli.host,
            args,
            "api/sparkplane/v1/doctor",
            render_doctor,
        ),
        SparkCommand::Qualify(args) => dispatch_qualification(&cli.host, args),
        ...
        SparkCommand::Cert(CertArgs {
            ...
            &cli.host,
            args,
            "api/sparkplane/v1/certificates/status",
            render_certificate,
        ),
        SparkCommand::Cert(CertArgs {
...
fn dispatch_client_migration(host: &str, args: ClientMigrationArgs) -> Result<(), SparkError> {
    if host != "local" || args.dry_run == args.yes {
        return Err(SparkError::usage(
            "client migration requires host 'local' and exactly one of --dry-run or --yes",
        ));
    }
    let destination = args.destination.unwrap_or_else(default_config_dir);
    let source = args
        .source
        .unwrap_or_else(|| destination.parent().unwrap_or(&destination).join("sy"));
    let report = crate::client_migration::migrate(&source, &destination, args.dry_run)
        .map_err(|error| SparkError::usage(format!("client migration rejected: {error:#}")))?;
    if args.json {
        print!("{}", render_json(&report)?);
    } else {
        println!(
            "{} {} Spark client files; source preserved",
            if args.dry_run {
                "Would import"
            } else {
                "Imported"
...
fn dispatch_qualification(host: &str, args: QualificationArgs) -> Result<(), SparkError> {
    if args.dry_run == args.yes {
        return Err(SparkError::usage(
            "qualification requires exactly one of --dry-run or --yes",
        ));
    }
    let manifest = fs::read(&args.manifest)
        .map_err(|error| SparkError::usage(format!("read qualification manifest: {error}")))?;
    if manifest.is_empty() || manifest.len() > 64 * 1024 {
        return Err(SparkError::usage(
            "qualification manifest must contain 1..65536 bytes",
        ));
    }
    let signature = fs::read_to_string(&args.signature)
        .map_err(|error| SparkError::usage(format!("read qualification signature: {error}")))?;
    if signature.is_empty() || signature.len() > 4 * 1024 {
        return Err(SparkError::usage(
            "qualification signature must contain 1..4096 bytes",
        ));
    }
    let submission = super::qualification::QualificationSubmission {
        manifest_base64: BASE64.encode(manifest),
        signature,
...
fn render_instance_human(instance: &super::wire::InstanceDocument) -> String {
    format!(
        ...
        instance.artifacts.primary.path,
        instance.artifacts.format,
        instance
            .artifacts
            .quantization
            .as_deref()
...
fn operator_logs<'a>(
    ...
    OperatorLogDocument {
        schema: "sparkplane.engine-logs/v2",
    ...
    let rows = document
        ...
        .map(|model| {
            ...
            [
                ...
                crate::human_bytes(model.logical_bytes),
...
fn render_model_list_json(document: &ModelListDocument) -> Result<String, SparkError> {
    ...
    render_json(&OperatorModelListDocument {
        schema: "sparkplane.model-list/v2",
        models,
    })
}
...
fn render_download_plan_human(
    ...
    Ok(format!(
        ...
        artifacts.format,
        artifacts.quantization.as_deref().unwrap_or("unknown"),
        fingerprint,
        plan.logical_bytes,
        plan.unique_bytes,
        plan.temporary_bytes,
        plan.disk_reserve_bytes,
    ))
}
    ...
    if let Some(OperationsCommand::Cancel(cancel)) = args.command {
        ...
        if cancel.dry_run {
            return render_or_print(
                cancel.json,
                &serde_json::json!({"schema":"sparkplane.dry-run/v1","action":"operation.cancel","operation_id":cancel.id}),
            );
        }
    ...
    match args.command {
        ...
        TokenCommand::Revoke(args) => {
            if args.dry_run {
                return render_or_print(
                    args.json,
                    &serde_json::json!({"schema":"sparkplane.dry-run/v1","action":"token.revoke","token_id":args.id}),
                );
            }
            if !args.yes {
                return Err(SparkError::usage(
    ...
    if resolved.yes {
        ...
        if operation == "upgrade" {
            let healthy = SparkClient::load(&resolved.config_dir, &manifest.host_alias)
                .and_then(|client| client.get_json::<StatusDocument>("api/sparkplane/v1/status"))
...
fn dispatch_bootstrap(host: String) -> Result<(), SparkError> {
    if host != "bootstrap" {
        return Err(SparkError::usage(
            "the inspect entrypoint is fixed to `sparkplane bootstrap inspect`",
        ));
    }
    #[cfg(feature = "appliance")]
    {
...
#[cfg(test)]
mod tests {
    use super::{InstallArgs, MapEnv, render_json, resolve_install};
    use clap::Parser;
    use std::{collections::BTreeMap, path::PathBuf};

    fn test_artifacts() -> crate::spark::wire::ModelArtifactsDocument {
        serde_json::from_str(concat!(
            r#"{"schema":"sparkplane.model-artifacts/v2","format":"gguf","primary":{"path":"model.gguf","bytes":8,"sha256":"#,
    ...
    fn test_model(
        ...
        crate::spark::wire::ModelDocument {
            schema: "sparkplane.model/v1".into(),
            id: "model-id".into(),
    ...
    fn test_instance(
        ...
        crate::spark::wire::InstanceDocument {
            schema: "sparkplane.instance/v2".into(),
            id: "instance-id".into(),
            name: "model".into(),
    ...
    fn model_list_is_one_compact_ollama_style_table() {
        ...
        let document = crate::spark::wire::ModelListDocument {
            schema: "sparkplane.model-list/v1".into(),
            models: vec![model],
        };
    ...
    fn model_list_uses_repository_when_alias_is_absent() {
        ...
        assert!(
            super::render_model_list_human(&document)
                .unwrap()
    ...
    fn process_list_contains_only_active_lifecycle_instances() {
        ...
        let filtered = super::active_instances(crate::spark::wire::InstanceListDocument {
            schema: "sparkplane.instance-list/v1".into(),
    ...
    fn json_documents_remain_machine_readable() {
        ...
        let logs = crate::spark::wire::EngineLogDocument {
            schema: "sparkplane.engine-logs/v1".into(),
    ...
    fn install_dry_run_obeys_flag_env_default_precedence() {
        let env = MapEnv(BTreeMap::from([
            ("SPARKPLANE_PROBE".into(), "/env/probe".into()),
            ("SPARKPLANE_LISTEN_ADDRESS".into(), "10.1.30.9".into()),
            ("SPARKPLANE_LISTEN_PORT".into(), "9443".into()),
        ]));
        ...
        assert_eq!(
            defaults.probe,
            PathBuf::from("/usr/local/share/sparkplane/appliance-release/sparkplane-aarch64")
        );
    ...
    fn install_output_keeps_approval_hash_and_reports_only_remote_changes() {
        use crate::spark::{
            install::{PlanOptions, build_manifest, manifest_approval_sha256},
            wire::{InstallExecution, decode_inventory},
        };
        let inventory =
            ...
            PlanOptions {
                ...
                catalogs: crate::spark::install::CatalogDigests {
                    ...
                    engines: std::collections::BTreeMap::from([
                        ("configs/sparkplane/engines/first.toml".into(), "l".into()),
                        ("configs/sparkplane/engines/second.toml".into(), "v".into()),
                    ]),
                },
            },
        )
        ...
        super::record_install_execution(
            &mut manifest,
            false,
            "opt/sparkplane/releases/0.1.0".into(),
    ...
    fn admission_human_output_exposes_the_complete_selected_fallback() {
        ...
        let report = AdmissionReport {
            schema: "sparkplane.admission-report/v1".into(),
            admitted: true,
            ...
            selection: Some(AdmissionSelection {
                ...
                artifacts: serde_json::from_str(r#"{"schema":"sparkplane.model-artifacts/v2","format":"safetensors","primary":{"path":"model.safetensors","bytes":8,"sha256":null},"auxiliary":[],"quantization":"FP8","capabilities":["text_generation"],"configured_alias":null}"#).unwrap(),
    ...
    #[test]
    fn qualification_command_accepts_only_signed_manifest_inputs() {
        #[derive(clap::Parser)]
        struct TestCli {
            #[command(flatten)]
            spark: super::SparkCli,
