...
use super::wire::{
    Applicability, AssetKind, ContentIdentity, ExecutionPhase, HostInventory, InstallExecution,
    InstallManifest, MANIFEST_SCHEMA, MigrationPlan, PlannedAsset, ProbeEvidence,
    ProtectedFingerprint, RejectedUpdateClass, RollbackPlan, ServiceTransition, decode_inventory,
};

#[cfg(feature = "appliance")]
const MODELS_BUNDLE_PATH: &str = "configs/sparkplane/models.toml";
const ENGINES_BUNDLE_PREFIX: &str = "configs/sparkplane/engines/";
...
pub fn inspect_and_plan(
    ...
    let remote_path = format!("/tmp/sparkplane-bootstrap-{probe_sha256}");
...
fn installed_engine_path(relative: &str) -> Result<PathBuf, InstallError> {
    ...
    Ok(PathBuf::from("etc/sparkplane/engines").join(name))
}
...
fn read_release_manifest(
    ...
    verify_signed_entry(&text, "sparkplane-aarch64", &executable)?;
    let root = path
        .parent()
...
pub fn load_release_payload(
    ...
    let (models, models_sha) = release_asset(&manifest, &root, "configs/sparkplane/models.toml")?;
...
fn planned_assets(
    ...
    for (path, owner, mode, applicability) in [
        ...
        (
            "/opt/sparkplane/hf-http-fallback",
            "root:root",
            "0755",
            apply_remote(),
        ),
        ("/etc/sparkplane", "root:sparkplane", "0750", apply_remote()),
        ("/etc/sparkplane", "root:sparkplane", "0750", apply_remote()),
        (
        ...
        (
            "/var/lib/sparkplane/executor",
            "root:sparkplane",
            "0750",
            apply_remote(),
        ),
        ("/run/sparkplane", "root:sparkplane", "0750", apply_remote()),
    ] {
        assets.push(asset(
            AssetKind::Directory,
            path,
    ...
    assets.extend([
        ...
        asset(
            AssetKind::File,
            &format!("{release_path}/sparkplane"),
            "root:root",
            "0555",
        ...
        asset(
            AssetKind::File,
            &format!("/opt/sparkplane/hf-http-fallback/{fallback_digest}/requirements.lock"),
            "root:root",
            "0444",
            ContentIdentity::SignedReleaseManifest,
            apply_remote(),
        ...
        asset(
            AssetKind::File,
            "/var/lib/sparkplane/executor/emergency.jsonl",
            "root:root",
            "0600",
            ContentIdentity::GeneratedAtInstall,
            deferred(3),
    ...
    for (relative, digest) in &catalogs.engines {
        ...
        assets.push(asset(
            AssetKind::File,
            &format!("/etc/sparkplane/engines/{name}"),
            "root:sparkplane",
            "0640",
    ...
    for (unit, applicability) in [
        ...
        ("/etc/systemd/system/sparkplane.target", apply_remote()),
    ] {
        assets.push(asset(
...
fn planned_service_transitions() -> Vec<ServiceTransition> {
    [
        ("sparkplane-executor.service", apply_remote()),
        ("sparkplane-agent.service", apply_remote()),
        ("sparkplane.target", apply_remote()),
    ]
    .into_iter()
...
pub fn rollback_release(
    ...
    let current_path = root.join("opt/sparkplane/current");
    let previous_path = root.join("opt/sparkplane/previous");
    ...
    let mut report = MaintenanceReport {
        schema: "sparkplane.maintenance/v1".into(),
...
fn checked_release_link(root: &Path, path: &Path) -> Result<PathBuf, InstallError> {
    ...
    let binary = root.join("opt/sparkplane").join(&target).join("sparkplane");
    ...
    let release_root = root.join("opt/sparkplane").join(&target);
...
fn hash_release_file(root: &Path, release: &Path, relative: &str) -> Result<String, InstallError> {
    let path = root.join("opt/sparkplane").join(release).join(relative);
    let bytes = std::fs::read(&path).map_err(|error| {
        configuration_error(format!("read release payload {}: {error}", path.display()))
    })?;
...
fn materialize_release_catalogs(root: &Path, release: &Path) -> Result<(), InstallError> {
    let base = root.join("opt/sparkplane").join(release);
    ...
    let installed = root.join("etc/sparkplane/engines");
        ...
        .map_err(|error| configuration_error(format!("read rollback engine inventory: {error}")))?
    {
        ...
        let relative = PathBuf::from("etc/sparkplane/engines").join(entry.file_name());
    ...
    let mut catalogs = vec![(
        ...
        PathBuf::from("etc/sparkplane/models.toml"),
    )];
...
fn validate_rollback_state(
    ...
    let database = root.join("var/lib/sparkplane/state.sqlite3");
    if !database.exists() {
    ...
    let engine_path = root.join("etc/sparkplane/engines");
...
fn quarantine_incompatible_state(root: &Path) -> Result<StateQuarantineEvidence, InstallError> {
    ...
    if !database.exists() {
        return Ok(StateQuarantineEvidence {
            schema: "sparkplane.state-quarantine/v1",
            backup: "absent".into(),
...
fn incompatible_instance_ids(
    ...
    for row in rows {
        ...
        if serde_json::from_str::<super::wire::InstanceDocument>(&metadata).is_err() {
            if desired == "running" {
                return Err(configuration_error(format!(
                    "incompatible running instance {id} must be stopped before authoritative replacement"
                )));
            }
...
fn backup_incompatible_state(
    ...
    let directory = root.join("var/lib/sparkplane/backups");
...
pub fn rotate_certificate(
    ...
    let ca_cert_path = root.join("var/lib/sparkplane/ca/ca-cert.pem");
    let ca_key_path = root.join("var/lib/sparkplane/ca/ca-key.pem");
    let leaf_cert_path = root.join("var/lib/sparkplane/tls/server-chain.pem");
    let leaf_key_path = root.join("var/lib/sparkplane/tls/server-key.pem");
    ...
    ca_params
        .distinguished_name
        .push(DnType::CommonName, "Sparkplane local CA");
    ...
    let mut report = CertificateRotationReport {
        schema: "sparkplane.certificate-rotation/v1".into(),
        dry_run,
        applied: false,
...
fn installed_listener_identity(root: &Path) -> Result<(String, String), InstallError> {
    let config = std::fs::read_to_string(root.join("etc/sparkplane/agent.toml"))
...
fn install_release_with_integration(
    ...
    let release_rel = PathBuf::from("opt/sparkplane/releases").join(&release_name);
    let release = root.join(&release_rel);
    let current = root.join("opt/sparkplane/current");
    let previous = root.join("opt/sparkplane/previous");
    let preceding_release = std::fs::read_link(&current).ok();
    if release.exists() {
        let installed = std::fs::read(release.join("sparkplane"))
    ...
    let mut report = InstallReport {
        changed: true,
        actions: vec![
            InstallAction::EnsureIdentity("user:sparkplane".into()),
            InstallAction::EnsureIdentity("group:sparkplane".into()),
        ],
        fsync_trace: Vec::new(),
    ...
    if !release.exists() {
        let stage = root.join("opt/sparkplane/releases").join(format!(
            ".stage-{}-{}",
            release_name,
            uuid::Uuid::new_v4()
        ));
        ...
        sync_dir(
            &root.join("opt/sparkplane/releases"),
    ...
    report.actions.push(InstallAction::VerifyReleaseArtifact(
        release_rel.join("sparkplane"),
    ));
    ...
    sync_dir(&root.join("opt/sparkplane"), &mut report.fsync_trace)?;
    report
        .actions
        .push(InstallAction::ActivateRelease(PathBuf::from(
            "opt/sparkplane/current",
    ...
    report.actions.push(InstallAction::ApplyServiceTransition(
        "sparkplane-executor.service".into(),
    ));
    report.actions.push(InstallAction::ApplyServiceTransition(
        "sparkplane-agent.service".into(),
    ));
    report.actions.push(InstallAction::ApplyServiceTransition(
        "sparkplane.target".into(),
    ));
    Ok((report, material))
}
...
impl TransactionSnapshot {
    fn capture(
        ...
        let mut file_paths = vec![
            "etc/sparkplane/agent.toml".into(),
            "etc/sparkplane/executor.toml".into(),
            "etc/sparkplane/models.toml".into(),
            "etc/systemd/system/sparkplane-agent.service".into(),
            "etc/systemd/system/sparkplane-executor.service".into(),
            "etc/systemd/system/sparkplane.target".into(),
            "etc/apparmor.d/sparkplane-agent".into(),
            "etc/apparmor.d/sparkplane-executor".into(),
            "var/lib/sparkplane/ca/ca-key.pem".into(),
            "var/lib/sparkplane/ca/ca-cert.pem".into(),
            "var/lib/sparkplane/tls/server-key.pem".into(),
            "var/lib/sparkplane/tls/server-chain.pem".into(),
            "etc/sparkplane/bootstrap-admin.credential".into(),
            "etc/sparkplane/hf-read.credential".into(),
        ];
        file_paths.extend(
            engines
                .iter()
        ...
        let previous = std::fs::read_link(root.join("opt/sparkplane/previous")).ok();
...
fn ensure_service_identity() -> Result<(), InstallError> {
    if !fixed_success("getent", &["group", "sparkplane"])? {
        require_fixed_success(
            "create sparkplane service group",
            "groupadd",
            &["--system", "sparkplane"],
        )?;
    }
    if !fixed_success("getent", &["passwd", "sparkplane"])? {
        require_fixed_success(
            "create sparkplane service user",
            "useradd",
            &[
                "--system",
                "--gid",
...
fn installed_service_uid(root: &Path) -> Result<u32, InstallError> {
    ...
    passwd
        ...
        .find_map(|line| {
            ...
            (fields.next() == Some("sparkplane"))
                .then(|| fields.nth(1)?.parse::<u32>().ok())
                .flatten()
        })
        .ok_or_else(|| configuration_error("installed sparkplane identity has no numeric UID"))
}
...
fn apply_host_integration() -> Result<(), InstallError> {
    ...
    require_fixed_success(
        ...
        "chown",
        &["sparkplane:sparkplane", "/var/lib/sparkplane"],
    )?;
    require_fixed_success(
    ...
    require_fixed_success(
        ...
        "chown",
        &["-R", "root:sparkplane", "/etc/sparkplane/engines"],
    )?;
    require_fixed_success(
...
fn restart_control_plane() -> Result<(), InstallError> {
    require_fixed_success(
        ...
        "systemctl",
        &["restart", "sparkplane-executor.service"],
    )?;
    require_fixed_success(
        "restart Spark agent on the activated release",
        "systemctl",
        &["restart", "sparkplane-agent.service"],
    )
}
...
pub(crate) fn ensure_http_fallback() -> Result<(), InstallError> {
    ...
    let root = PathBuf::from("/opt/sparkplane/hf-http-fallback");
...
pub fn activate_from_files(
    ...
    validate_catalog_approval(&manifest, "/etc/sparkplane/models.toml", &digests.models)?;
...
pub fn write_bootstrap_channel(
    ...
    let metadata = serde_json::to_vec(&ActivationMetadata {
        schema: "sparkplane.activation-result/v1".into(),
...
const SUDO_PROMPT: &str = "SPARKPLANE_SUDO_PROMPT:";
...
pub fn activate_over_ssh(request: &RemoteActivation<'_>) -> Result<ActivationResult, InstallError> {
    ...
    let local_manifest = std::env::temp_dir().join(format!(
        "sparkplane-manifest-{manifest_sha256}-{}.json",
        uuid::Uuid::new_v4()
    ));
    ...
    let mut batch = format!(
        ...
        quote_sftp_path(request.executable)?,
        quote_sftp_path(request.signature)?,
        quote_sftp_path(request.public_key)?,
        quote_sftp_path(local_manifest.path())?,
        quote_sftp_path(request.release_manifest)?,
        quote_sftp_path(request.models)?,
    );
...
fn activation_remote_prefix(executable_sha256: &str) -> Result<String, InstallError> {
    ...
    Ok(format!("/tmp/sparkplane-bootstrap-{executable_sha256}"))
}
...
pub fn maintenance_over_ssh(
    ...
    let mut args = vec![
        ...
        SUDO_PROMPT.into(),
        "--".into(),
        "/opt/sparkplane/current/sparkplane".into(),
        "bootstrap".into(),
    ];
    match command {
    ...
    rotate_certificate(Path::new("/"), dry_run, rotate_ca, || {
        if dry_run {
            ...
            require_fixed_success(
                ...
                "systemctl",
                &["kill", "--signal=HUP", "sparkplane-agent.service"],
            )
        }
    })
}
...
fn parse_bootstrap_channel(bytes: &[u8]) -> Result<ActivationResult, InstallError> {
    ...
    if metadata.schema != "sparkplane.activation-result/v1" {
...
pub(crate) fn validate_release(bundle: &ReleaseBundle<'_>) -> Result<(), InstallError> {
    ...
    verify_signed_entry(manifest, "sparkplane-aarch64", bundle.executable)?;
    verify_signed_entry(manifest, "configs/sparkplane/models.toml", bundle.models)?;
...
fn validate_shipped_policy_assets(bundle: &ReleaseBundle<'_>) -> Result<(), InstallError> {
    let agent: super::agent::AgentConfig =
        toml::from_str(include_str!("../../configs/sparkplane/agent.toml")).map_err(|error| {
            configuration_error(format!("validate shipped agent policy: {error}"))
        })?;
    let executor: super::executor::ExecutorConfig = toml::from_str(include_str!(
        "../../configs/sparkplane/executor.toml"
    ))
    .map_err(|error| configuration_error(format!("validate shipped executor policy: {error}")))?;
    if agent.schema != "sparkplane.agent/v1"
        || agent.model_catalog != Path::new("/etc/sparkplane/models.toml")
        || agent.engine_catalog != Path::new("/etc/sparkplane/engines")
        || executor.schema != "sparkplane.executor/v1"
        || executor.engine_catalog != agent.engine_catalog
        || executor.resources_policy != Path::new("/etc/sparkplane/agent.toml")
...
fn directory_layout() -> [(&'static str, u32, bool); 14] {
    [
        ("opt/sparkplane", 0o755, true),
        ("opt/sparkplane/releases", 0o755, true),
        ("opt/sparkplane/hf-http-fallback", 0o755, true),
        ("etc/sparkplane", 0o750, true),
        ("etc/sparkplane/engines", 0o750, true),
        ("etc/apparmor.d", 0o755, false),
        ("etc/systemd/system", 0o755, false),
        ("var/lib/sparkplane", 0o750, true),
        ("var/lib/sparkplane/huggingface", 0o750, true),
        ("var/lib/sparkplane/compile-cache", 0o750, true),
        ("var/lib/sparkplane/executor", 0o750, true),
        ("var/lib/sparkplane/tls", 0o700, true),
        ("var/lib/sparkplane/ca", 0o700, true),
        ("run/sparkplane", 0o750, true),
    ]
}
...
fn write_policy_assets(
    ...
    let config = include_str!("../../configs/sparkplane/agent.toml").replace(
        "10.1.30.143:9843",
        &format!("{}:9843", bundle.listen_address),
    );
    let executor_config = include_str!("../../configs/sparkplane/executor.toml")
        .replace("agent_uid = 996", &format!("agent_uid = {service_uid}"));
    for (relative, bytes, mode) in [
        ("etc/sparkplane/agent.toml", config.as_bytes(), 0o640),
        ("etc/sparkplane/models.toml", bundle.models, 0o640),
        (
            "etc/sparkplane/executor.toml",
        ...
        (
            "etc/systemd/system/sparkplane-agent.service",
            include_bytes!("../../configs/systemd/system/sparkplane-agent.service").as_slice(),
            0o644,
        ),
        (
            "etc/systemd/system/sparkplane-executor.service",
            include_bytes!("../../configs/systemd/system/sparkplane-executor.service").as_slice(),
            0o644,
        ),
        (
            "etc/systemd/system/sparkplane.target",
            include_bytes!("../../configs/systemd/system/sparkplane.target").as_slice(),
            0o644,
        ),
        (
            "etc/apparmor.d/sparkplane-agent",
            include_bytes!("../../configs/apparmor.d/sparkplane-agent").as_slice(),
            0o644,
        ),
        (
            "etc/apparmor.d/sparkplane-executor",
            include_bytes!("../../configs/apparmor.d/sparkplane-executor").as_slice(),
            0o644,
        ),
    ] {
    ...
    let desired = bundle
        ...
        .collect::<Result<BTreeMap<_, _>, InstallError>>()?;
    let engine_dir = root.join("etc/sparkplane/engines");
    ...
    let fallback_lock = include_bytes!("../../configs/sparkplane/hf-http-fallback.lock");
    let fallback_digest = format!("{:x}", Sha256::digest(fallback_lock));
    let fallback_dir = root
        .join("opt/sparkplane/hf-http-fallback")
...
fn ensure_local_identity(
    ...
    let leaf_cert_path = root.join("var/lib/sparkplane/tls/server-chain.pem");
    let leaf_key_path = root.join("var/lib/sparkplane/tls/server-key.pem");
    let token_path = root.join("etc/sparkplane/bootstrap-admin.credential");
    let hf_token_path = root.join("etc/sparkplane/hf-read.credential");
...
fn existing_bootstrap_material(root: &Path) -> Result<BootstrapMaterial, InstallError> {
    let ca_certificate_pem = std::fs::read_to_string(
        root.join("var/lib/sparkplane/ca/ca-cert.pem"),
    )
    .map_err(|error| configuration_error(format!("read installed CA certificate: {error}")))?;
    let token = std::fs::read_to_string(root.join("etc/sparkplane/bootstrap-admin.credential"))
        .map_err(|error| {
...
pub fn bootstrap_inventory() -> Result<HostInventory, InstallError> {
    use super::wire::{
        DockerInventory, ExistingInstallation, GpuInventory, INVENTORY_SCHEMA, OsInventory,
        PythonInventory,
    };
    ...
    let expected_name = format!("sparkplane-bootstrap-{probe_sha256}");
    ...
    let python_version = fixed_output("python3", &["--version"])?
        ...
        .to_string();
    let current_release = std::fs::read_link("/opt/sparkplane/current")
        .ok()
        .map(|path| path.to_string_lossy().into_owned());
    ...
    for interface in value.as_array().into_iter().flatten() {
        ...
        for address in interface["addr_info"].as_array().into_iter().flatten() {
            if address["family"] == "inet"
                ...
                && let Some(local) = address["local"].as_str()
            {
...
fn validate_remote_probe_path(path: &str) -> Result<(), String> {
    const PREFIX: &str = "/tmp/sparkplane-bootstrap-";
    let digest = path
        .strip_prefix(PREFIX)
...
mod tests {
    use super::{
        BootstrapRunner, InspectInvocation, OpenSshRunner, PlanOptions, ProbeTransfer, RunnerError,
        build_manifest,
    };
    use crate::spark::wire::{AssetKind, ContentIdentity, decode_inventory};
    use sha2::Digest;
    use std::path::PathBuf;

    fn shipped_engine_paths() -> Vec<PathBuf> {
        let mut paths = std::fs::read_dir("configs/sparkplane/engines")
            .unwrap()
            .filter_map(Result::ok)
            .map(|entry| entry.path())
    ...
    fn test_release_manifest_for(
        ...
        let mut manifest = format!(
            ...
            digest(executable),
            digest(include_bytes!("../../configs/sparkplane/models.toml")),
        );
    ...
    fn test_catalog_digests() -> super::CatalogDigests {
        super::CatalogDigests {
            models: format!(
                "{:x}",
                sha2::Sha256::digest(include_bytes!("../../configs/sparkplane/models.toml"))
            ),
    ...
    fn release_payload_discovers_every_signed_engine_file() {
        ...
        let engines = root.path().join("configs/sparkplane/engines");
        std::fs::create_dir_all(&engines).unwrap();
        let executable = b"arm64";
        let models = b"models";
        std::fs::write(root.path().join("sparkplane-aarch64"), executable).unwrap();
        std::fs::create_dir_all(root.path().join("configs/sparkplane")).unwrap();
        std::fs::write(root.path().join("configs/sparkplane/models.toml"), models).unwrap();
        let mut entries = vec![
        ...
        for (name, bytes) in entries {
            ...
            manifest.push_str(&format!(
                "{}  configs/sparkplane/engines/{name}\n",
                digest(bytes)
            ));
        }
        ...
        let payload =
            super::load_release_payload(&manifest_path, &root.path().join("sparkplane-aarch64"))
                .unwrap();
        ...
        assert_eq!(
            payload
                ...
                .collect::<Vec<_>>(),
            [
                "configs/sparkplane/engines/first.toml",
                "configs/sparkplane/engines/second.toml",
                "configs/sparkplane/engines/third.toml",
            ]
        );
    }
    ...
    fn staged_engine_arguments_reject_paths_outside_the_signed_inventory() {
        assert!(
            super::parse_staged_engine_asset(
                "configs/sparkplane/engines/third.toml=/tmp/third.toml"
            )
            .is_ok()
        );
        for value in [
            "configs/sparkplane/engines/nested/third.toml=/tmp/third.toml",
            "configs/sparkplane/engines/../third.toml=/tmp/third.toml",
            "configs/sparkplane/engines/third.toml=relative.toml",
        ] {
    ...
    fn signed_bundle<'a>(
        ...
        super::ReleaseBundle {
            ...
            models: include_bytes!("../../configs/sparkplane/models.toml"),
    ...
    fn host_alias_and_paths_are_discrete_argv() {
        let transfer = ProbeTransfer {
            ...
            remote_path: format!("/tmp/sparkplane-bootstrap-{}", "a".repeat(64)),
        };
        ...
        assert_eq!(&inspect_args[3..], ["bootstrap", "inspect"]);
    }
    ...
    fn activation_process_inherits_terminal_input_and_captures_only_protocol_output() {
        const PREFIX: &str = "/tmp/sparkplane-release-abc";
        let signature = format!("{PREFIX}.minisig");
        let public_key = format!("{PREFIX}.pub");
        let manifest = format!("{PREFIX}.manifest.json");
        let engines = vec![
            (
                "configs/sparkplane/engines/first.toml".into(),
                "/tmp/first.toml".into(),
            ),
            (
                "configs/sparkplane/engines/second.toml".into(),
        ...
        assert_eq!(
            ...
            [
                ...
                "/tmp/models.toml",
                "--engine",
                "configs/sparkplane/engines/first.toml=/tmp/first.toml",
                "--engine",
                "configs/sparkplane/engines/second.toml=/tmp/second.toml",
                "--version",
                "0.1.0",
                "--listen-address",
                "10.1.30.143",
    ...
    fn activation_stages_the_executable_under_its_bootstrap_identity() {
        ...
        assert_eq!(
            ...
            format!("/tmp/sparkplane-bootstrap-{sha256}")
        );
    }
    ...
    fn capability_probe_output_is_not_written_to_wire_stream() {
        const REENTRY: &str = "SPARKPLANE_STATUS_PROBE_TEST";
        ...
        if std::env::var_os(REENTRY).is_some() {
            assert!(super::fixed_status(
                ...
                &[
                    "-c",
                    "printf spark-capability-probe-output; printf spark-capability-probe-output >&2"
                ]
            ));
            return;
        }
    ...
    fn manifest_installs_both_catalogs_with_expected_hashes() {
        ...
        let opts = PlanOptions {
            ...
            listen_port: 9843,
            probe_remote_path: format!("/tmp/sparkplane-bootstrap-{}", "a".repeat(64)),
        ...
        assert!(paths.contains(&format!("{release_path}/sparkplane").as_str()));
        assert!(paths.contains(&"/opt/sparkplane/current"));
        assert!(paths.contains(&"/var/lib/sparkplane/state.sqlite3"));
        assert!(paths.contains(&"/etc/systemd/system/sparkplane-agent.service"));
        assert!(paths.contains(&"/etc/systemd/system/sparkplane-executor.service"));
        assert!(paths.contains(&"/etc/systemd/system/sparkplane.target"));
        assert!(
            manifest
                .assets
                .iter()
        ...
        assert!(paths.contains(&"/etc/sparkplane/models.toml"));
        let models = manifest
            .assets
            .iter()
            .find(|asset| asset.path_or_name == "/etc/sparkplane/models.toml")
            .unwrap();
        assert_eq!(
            models.content,
    ...
    fn catalog_validation_rejects_mutable_images_and_malformed_models() {
        const MODELS: &str = include_str!("../../configs/sparkplane/models.toml");
        ...
        changed[0].1 = &mutable;
        let malformed = MODELS.replace("schema = \"sparkplane.models/v2\"", "schema = \"old\"");
    ...
    fn first_install_then_reapply_is_atomic_and_idempotent() {
        ...
        let root = tempfile::tempdir().unwrap();
        let executable = b"signed-aarch64-sparkplane-release";
        let keys = KeyPair::generate_unencrypted_keypair().unwrap();
        let public_key = keys.pk.to_base64();
        let signature = minisign::sign(
            None,
        ...
        assert_eq!(
            std::fs::read_link(root.path().join("opt/sparkplane/current")).unwrap(),
            PathBuf::from("releases").join(&release_name)
        );
        assert_eq!(
            super::checked_release_link(root.path(), &root.path().join("opt/sparkplane/current"))
                .unwrap(),
            PathBuf::from("releases").join(&release_name)
        );
        assert_eq!(
            std::fs::metadata(
                root.path()
                    .join("etc/sparkplane/bootstrap-admin.credential")
            )
            .unwrap()
            .permissions()
            .mode()
        ...
        let release_parent_sync = first
            .fsync_trace
            .iter()
            .position(|path| path.ends_with("opt/sparkplane/releases"))
            .unwrap();
        ...
        assert!(
            first
                .fsync_trace
                .iter()
                .any(|path| path.ends_with("opt/sparkplane"))
        );
    ...
    fn same_version_different_signed_artifacts_coexist_without_overwrite() {
        ...
        assert_eq!(
            std::fs::read_link(root.path().join("opt/sparkplane/previous")).unwrap(),
    ...
    fn upgrade_replaces_shipped_catalogs_without_legacy_merge() {
        ...
        for (version, executable) in [
            ...
            super::install_release(root.path(), &bundle).unwrap();
            if version == "0.1.0" {
                std::fs::write(root.path().join("etc/sparkplane/models.toml"), "stale").unwrap();
        ...
        assert_eq!(
            std::fs::read(root.path().join("etc/sparkplane/models.toml")).unwrap(),
            include_bytes!("../../configs/sparkplane/models.toml")
        );
        assert_eq!(
        ...
        assert!(!root.path().join("etc/sparkplane/recipes.d").exists());
    }
    ...
    fn upgrade_rejects_stale_schema_and_unavailable_engine_identity() {
        ...
        let root = tempfile::tempdir().unwrap();
        let state_dir = root.path().join("var/lib/sparkplane");
        ...
        let instance = serde_json::json!({
            "schema":"sparkplane.instance/v2","id":format!("i_{}", "1".repeat(32)),
            ...
            "artifacts":{"schema":"sparkplane.model-artifacts/v2","format":"gguf","primary":{"path":"model.gguf","bytes":1,"sha256":null},"auxiliary":[],"quantization":"Q4_K_M","capabilities":["text_generation"],"configured_alias":null},
        ...
        connection
            ...
            .unwrap();
        let engines = root.path().join("etc/sparkplane/engines");
        std::fs::create_dir_all(&engines).unwrap();
        let documents = shipped_engine_documents();
    ...
    fn upgrade_quarantines_only_stopped_incompatible_state() {
        ...
        let root = tempfile::tempdir().unwrap();
        let state = root.path().join("var/lib/sparkplane");
        std::fs::create_dir_all(state.join("backups")).unwrap();
        let connection = Connection::open(state.join("state.sqlite3")).unwrap();
        connection.execute_batch("PRAGMA user_version=6; CREATE TABLE models(id TEXT PRIMARY KEY,repository TEXT,commit_sha TEXT,metadata_json TEXT); CREATE TABLE aliases(name TEXT PRIMARY KEY,model_id TEXT); CREATE TABLE instances(id TEXT PRIMARY KEY,name TEXT,model_id TEXT,generation INTEGER,desired_state TEXT,observed_state TEXT,metadata_json TEXT); CREATE TABLE instance_resources(instance_id TEXT); CREATE TABLE restart_failures(instance_id TEXT); CREATE TABLE benchmarks(id TEXT PRIMARY KEY,model_id TEXT,metadata_json TEXT); CREATE TABLE audit(sequence INTEGER PRIMARY KEY AUTOINCREMENT,occurred_at TEXT,actor_token_id TEXT,action TEXT,target TEXT,outcome TEXT,metadata_json TEXT); INSERT INTO models VALUES('m_old','owner/model','aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa','{\"schema\":\"sparkplane.model/v1\"}'); INSERT INTO aliases VALUES('old','m_old'); INSERT INTO instances VALUES('i_old','old','m_old',1,'stopped','absent','{\"schema\":\"sparkplane.instance/v1\"}'); INSERT INTO audit(occurred_at,actor_token_id,action,target,outcome,metadata_json) VALUES('before','operator','keep','unrelated','success','{}');").unwrap();
    ...
    fn multi_engine_inventory_activates_and_rolls_back_atomically() {
        ...
        assert_eq!(
            std::fs::read_link(root.path().join("opt/sparkplane/current")).unwrap(),
            prior_release.strip_prefix("opt/sparkplane").unwrap()
        );
    ...
    fn rollback_swaps_only_control_plane_release_links() {
        ...
        let root = tempfile::tempdir().unwrap();
        let releases = root.path().join("opt/sparkplane/releases");
        ...
        for (name, bytes) in [
            ...
            std::fs::create_dir_all(releases.join(name).join("configs/sparkplane/engines"))
                .unwrap();
            std::fs::write(releases.join(name).join("sparkplane"), bytes).unwrap();
            std::fs::write(
        ...
        symlink(
            ...
            root.path().join("opt/sparkplane/current"),
        )
        .unwrap();
        symlink(
            PathBuf::from("releases").join(&first),
            root.path().join("opt/sparkplane/previous"),
        )
        .unwrap();
        std::fs::create_dir_all(root.path().join("etc/sparkplane/engines")).unwrap();
        std::fs::write(
            root.path().join("etc/sparkplane/engines/stale.toml"),
            "stale",
        )
        .unwrap();
        ...
        assert!(
            !root
                .path()
                .join("etc/sparkplane/engines/stale.toml")
                .exists()
        );
    }
    ...
    fn rollback_rejects_parent_directory_release_links() {
        ...
        let root = tempfile::tempdir().unwrap();
        std::fs::create_dir_all(root.path().join("opt/sparkplane")).unwrap();
        symlink("releases/..", root.path().join("opt/sparkplane/current")).unwrap();
        let error =
            super::checked_release_link(root.path(), &root.path().join("opt/sparkplane/current"))
                .unwrap_err();
    ...
    fn generated_ca_and_leaf_have_verifiable_server_certificate_extensions() {
        ...
        let ca_pem = std::fs::read(root.path().join("var/lib/sparkplane/ca/ca-cert.pem")).unwrap();
        let leaf_pem =
            std::fs::read(root.path().join("var/lib/sparkplane/tls/server-chain.pem")).unwrap();
    ...
    fn certificate_rotation_preserves_overlap_and_requires_repin_only_for_ca() {
        ...
        super::install_release(root.path(), &bundle).unwrap();
        let original_ca =
            std::fs::read(root.path().join("var/lib/sparkplane/ca/ca-cert.pem")).unwrap();
        let original_leaf =
            std::fs::read(root.path().join("var/lib/sparkplane/tls/server-chain.pem")).unwrap();
        ...
        assert_eq!(
            std::fs::read(
                root.path()
                    .join("var/lib/sparkplane/tls/server-chain.pem.overlap")
            )
            .unwrap(),
            original_leaf
        );
    ...
    fn approved_install_assets_and_executed_actions_are_one_to_one() {
        ...
        let manifest = build_manifest(
            ...
            PlanOptions {
                ...
                listen_address: "10.1.30.143".into(),
                listen_port: 9843,
                probe_remote_path: format!("/tmp/sparkplane-bootstrap-{sha256}"),
        ...
        assert!(manifest.assets.iter().any(|asset| {
            asset.path_or_name.contains("sparkplane-executor")
                && asset.applicability
                    == Applicability::ApplyNow {
                        phase: ExecutionPhase::RemoteInstall,
                    }
        ...
        assert!(
            super::validate_install_actions(&manifest, &report.actions[..report.actions.len() - 1])
                .is_err()
        );
    }
    ...
    fn activation_prompt_is_exact_and_contamination_is_rejected() {
        let report = super::InstallReport {
            ...
            active_release: PathBuf::from("opt/sparkplane/releases/0.1.0"),
        };
    ...
    fn integration_failure_removes_first_install_transaction_residue() {
        ...
        let sha256 = format!("{:x}", Sha256::digest(executable));
        let bundle = signed_bundle(executable, &public_key, &signature, &sha256);
        let release_path = format!("opt/sparkplane/releases/{}", test_release_name(&sha256));
        let root = tempfile::tempdir().unwrap();
    ...
    fn failed_reapply_restores_prior_activation_config_and_policy() {
        ...
        for (relative, prior) in [
            ("etc/sparkplane/agent.toml", b"prior-config".as_slice()),
            (
                "etc/sparkplane/executor.toml",
    ...
    fn spark_agent_systemd_unit_verifies_and_pins_hardening() {
        ...
        let profile = include_str!("../../configs/apparmor.d/sparkplane-agent");
        for directive in [
            "Type=notify",
            "User=sparkplane",
            "Group=sparkplane",
            ...
            "DevicePolicy=closed",
            "ReadWritePaths=/var/lib/sparkplane",
            "LimitNOFILE=4096",
            "TasksMax=128",
            "MemoryMax=256M",
            "OOMScoreAdjust=-100",
            ...
            "AppArmorProfile=sparkplane-agent",
        ] {
            assert!(
        ...
        if unit.contains("Type=notify") && unit.contains("WatchdogSec=") {
            for permission in [
                ...
                "/proc/*/cgroup r,",
                "/opt/sparkplane/hf-http-fallback/*/venv/pyvenv.cfg r,",
                "/opt/sparkplane/hf-http-fallback/*/venv/lib/** mr,",
                "/tmp/** rwkl,",
                "/dev/shm/sem.* rwkl,",
            ] {
                assert!(
        ...
        let output = std::process::Command::new(systemd_analyze)
            .args(["verify", "configs/systemd/system/sparkplane-agent.service"])
            .output()
            .unwrap();
        let hard_errors: Vec<_> = String::from_utf8_lossy(&output.stderr)
            .lines()
            .filter(|line| {
                !line.contains("/opt/sparkplane/current/sparkplane") || !line.contains("executable")
            })
    ...
    fn confinement_allows_catalog_reads_but_not_writes() {
        let agent_unit = include_str!("../../configs/systemd/system/sparkplane-agent.service");
        let executor_unit =
            include_str!("../../configs/systemd/system/sparkplane-executor.service");
        let agent_profile = include_str!("../../configs/apparmor.d/sparkplane-agent");
        let executor_profile = include_str!("../../configs/apparmor.d/sparkplane-executor");

        assert!(agent_unit.contains(
            "ReadOnlyPaths=/etc/sparkplane/agent.toml /etc/sparkplane/models.toml /etc/sparkplane/engines"
        ));
        assert!(executor_unit.contains("ReadOnlyPaths=/etc/sparkplane/executor.toml /etc/sparkplane/agent.toml /etc/sparkplane/engines /var/lib/sparkplane/huggingface"));
        assert!(agent_profile.contains("/etc/sparkplane/models.toml r,"));
        assert!(agent_profile.contains("/etc/sparkplane/engines/*.toml r,"));
        assert!(executor_profile.contains("/etc/sparkplane/engines/*.toml r,"));
        assert!(!executor_profile.contains("/etc/sparkplane/models.toml r,"));
        for profile in [agent_profile, executor_profile] {
            assert!(profile.contains("deny /etc/sparkplane/** wklx,"));
            assert!(!profile.contains("/etc/sparkplane/** rw"));
        }
    ...
    #[test]
    fn confined_executor_can_read_the_active_qualification_authority() {
        let profile = include_str!("../../configs/apparmor.d/sparkplane-executor");

        assert!(profile.contains("/opt/sparkplane/releases/*/minisign.pub r,"));
    }

    #[test]
    fn confined_agent_can_read_the_active_qualification_authority() {
        let profile = include_str!("../../configs/apparmor.d/sparkplane-agent");
    ...
    fn host_integration_grants_agent_access_to_engine_catalog() {
        assert!(super::EXECUTOR_BOUNDARY_CHOWN_ARGS.contains(&"/etc/sparkplane"));
        assert!(super::EXECUTOR_BOUNDARY_CHOWN_ARGS.contains(&"/etc/sparkplane/engines"));
        assert!(super::EXECUTOR_BOUNDARY_CHOWN_ARGS.contains(&"/etc/sparkplane/models.toml"));
    }

    #[test]
    fn spark_executor_unit_is_unix_only_group_reachable_and_hardened() {
        let unit = include_str!("../../configs/systemd/system/sparkplane-executor.service");
        let agent_unit = include_str!("../../configs/systemd/system/sparkplane-agent.service");
        let profile = include_str!("../../configs/apparmor.d/sparkplane-executor");
        for directive in [
            "Type=notify",
            ...
            "Before=sparkplane-agent.service",
            "RuntimeDirectory=sparkplane",
            ...
            "ReadOnlyPaths=/etc/sparkplane/executor.toml /etc/sparkplane/agent.toml /etc/sparkplane/engines /var/lib/sparkplane/huggingface",
            "ReadWritePaths=/run/sparkplane /var/run/docker.sock /var/lib/sparkplane/executor /var/lib/sparkplane/compile-cache /sys/fs/cgroup/system.slice",
            "MemoryMax=64M",
            "WatchdogSec=30s",
            "Restart=on-failure",
            "AppArmorProfile=sparkplane-executor",
        ] {
            assert!(
                unit.contains(directive),
                "missing executor hardening: {directive}"
            );
        }
        assert!(!unit.contains("AF_INET"));
        assert!(agent_unit.contains("Wants=network-online.target sparkplane-executor.service"));
        for permission in [
            "network unix stream,",
            "/etc/sparkplane/engines/*.toml r,",
            "/var/lib/sparkplane/huggingface/** r,",
            "/run/sparkplane/executor.sock rwk,",
        ...
        if let Ok(systemd_analyze) = which::which("systemd-analyze") {
            let output = std::process::Command::new(systemd_analyze)
                .args([
                    "verify",
                    "configs/systemd/system/sparkplane-executor.service",
                ])
                .output()
                .unwrap();
            let hard_errors = String::from_utf8_lossy(&output.stderr)
                .lines()
                .filter(|line| {
                    !line.contains("/opt/sparkplane/current/sparkplane")
    ...
    fn installer_materializes_the_actual_numeric_service_uid() {
        ...
        let mut report = super::InstallReport {
            ...
            fsync_trace: Vec::new(),
            preceding_release: None,
            active_release: PathBuf::from("opt/sparkplane/releases/test"),
        };
        let bundle = signed_bundle(b"fixture", "key", "signature", "digest");
        super::write_policy_assets(root.path(), &bundle, 4242, &mut report).unwrap();
        let config =
            std::fs::read_to_string(root.path().join("etc/sparkplane/executor.toml")).unwrap();
    ...
    fn installer_materializes_the_supervision_target() {
        ...
        let target =
            std::fs::read_to_string(root.path().join("etc/systemd/system/sparkplane.target"))
                .unwrap();
...
pub(crate) mod tests_fixture {
    pub const INVENTORY: &str = r#"{
        "schema":"sparkplane.bootstrap.inventory/v1","probe_sha256":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
