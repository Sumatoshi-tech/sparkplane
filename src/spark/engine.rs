...
fn validate_profiles(config: &EngineConfig) -> Result<(), String> {
    ...
    for profile in &config.profiles {
        ...
        if let Some(embedding) = &profile.embeddings
            && (!embedding.dimensions_from_model_config
...
fn valid_managed_root(value: &str) -> bool {
    ...
    path.is_absolute()
        && path.starts_with("/var/lib/sparkplane")
        && path != Path::new("/var/lib/sparkplane")
        && !path
            .components()
...
mod tests {
    ...
    fn every_shipped_engine_declaration_parses() {
        let directory = Path::new("configs/sparkplane/engines");
    ...
    fn declared_executable_caches_must_avoid_noexec_tmpfs() {
        let path = std::fs::read_dir("configs/sparkplane/engines")
            .unwrap()
    ...
    fn isolated_compile_cache_root_can_host_executable_temporaries() {
        let text = std::fs::read_dir("configs/sparkplane/engines")
            .unwrap()
            .filter_map(Result::ok)
    ...
    const LLAMA: &str = include_str!("../../configs/sparkplane/engines/llama-cpp.toml");
    const VLLM: &str = include_str!("../../configs/sparkplane/engines/vllm.toml");
    const VLLM_QWEN38: &str =
        include_str!("../../configs/sparkplane/engines/vllm-qwen38-mmap.toml");
    const VLLM_QWEN38_DOCKERFILE: &str =
        include_str!("../../configs/sparkplane/engines/vllm-qwen38-mmap.Dockerfile");
    ...
    fn shipped_freetoken_engine_is_declarative_and_spark_tuned() {
        let text = std::fs::read_to_string("configs/sparkplane/engines/freetoken.toml").unwrap();
    ...
    fn stream_idle_timeout_override_is_bounded() {
        let config = include_str!("../../configs/sparkplane/engines/vllm-qwen38-mmap.toml")
            .replace(
    ...
    fn patched_vllm_module_is_readable_by_non_root_runtime() {
        assert!(
            VLLM_QWEN38_DOCKERFILE.contains("chmod 0644 \"${SITE_PACKAGES}/vllm_ple_mmap.py\"")
        );
    }
    ...
    fn native_response_timeout_is_required_and_bounded() {
        let config = include_str!("../../configs/sparkplane/engines/freetoken.toml");
