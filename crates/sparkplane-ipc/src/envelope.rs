...
// `TraceId` and `SpanId` moved to `sparkplane_core::trace` in arch-observability
...
// imports `sparkplane_ipc::envelope::{TraceId, SpanId}` working unchanged.
// Wire format is unaffected: both types still serialise to a bare hex
// string via `#[serde(transparent)]`.
pub use sparkplane_core::trace::{SpanId, TraceId};
...
/// seal check lives in the consumer of `sparkplane-ipc`.
...
/// example). `code` is the wire-stable `ErrorCode` from `sparkplane-core`;
