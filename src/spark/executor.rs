...
use sparkplane_core::{ErrorCode, Priority};
use sparkplane_ipc::{
    CallOpts, CancelRegistry, Dispatched, ErrorBody, Handler, PeerAuthorizer, PeerCredentials,
    Request, Response, SCHEMA_VERSION, Server, dispatch_with_cancel,
};
...
use super::{
    ...
    engine::{
        EngineCatalog, EngineHealthBody, EngineModelMount, EnginePolicy, artifact_fingerprint,
    },
    gateway::GatewayProfile,
};
use super::{
    qualification::{
        QUALIFICATION_PLAN_SCHEMA, QualificationExecution, QualificationOperation,
        QualificationPlanDocument, QualificationRunInput, QualificationSubmission,
        QualificationTarget, RELEASE_PUBLIC_KEY_PATH,
    },
    resources::{
...
pub const EXECUTOR_SOCKET: &str = "/run/sparkplane/executor.sock";
const RESOURCE_POLICY_PATH: &str = "/etc/sparkplane/agent.toml";
const ENGINE_CATALOG_PATH: &str = "/etc/sparkplane/engines";
const COMPILE_CACHE_IDENTITY_SCHEMA: &str = "sparkplane.compile-cache-identity/v1";
...
const QUALIFICATION_CLEANUP_ACK_SECONDS: u64 = 10;
const QUALIFICATION_CLIENT_GRACE_SECONDS: u64 = QUALIFICATION_CLEANUP_ACK_SECONDS + 1;

fn qualification_response_timeout(deadline: Duration) -> Duration {
    deadline.saturating_add(Duration::from_secs(QUALIFICATION_CLIENT_GRACE_SECONDS))
}
...
struct QualificationContainerSpec {
    name: String,
    image: String,
    architecture: String,
    entrypoint: Vec<String>,
...
fn build_qualification_container_spec(
    plan: &QualificationPlanDocument,
    operation_id: &str,
) -> Result<QualificationContainerSpec, ()> {
    if plan.schema != QUALIFICATION_PLAN_SCHEMA
        || !plan.admitted
        || plan.resources.is_none()
        || Ulid::from_string(operation_id).is_err()
    {
        return Err(());
    }
    let labels = BTreeMap::from([
        ("io.sparkplane.qualification".into(), "true".into()),
        (
            "io.sparkplane.qualification.job".into(),
            plan.job_id.clone(),
        ),
        (
            "io.sparkplane.qualification.manifest".into(),
            plan.manifest_sha256.clone(),
        ),
        (
            "io.sparkplane.qualification.operation".into(),
            operation_id.into(),
        ),
    ]);
    let kernel_jit = matches!(
        plan.operation,
        QualificationOperation::SparkFlashQsaV1 | QualificationOperation::SparkFlashMoeV1
    );
    let mut environment = vec![
        "CUDA_VISIBLE_DEVICES=0".into(),
        "NVIDIA_VISIBLE_DEVICES=0".into(),
    ...
    let mut tmpfs = BTreeMap::from([(
        "/tmp".into(),
        "rw,nosuid,nodev,noexec,size=67108864,mode=1770,uid=65534,gid=65534".into(),
    )]);
    if kernel_jit {
        tmpfs.insert(
            "/tmp/triton".into(),
            "rw,nosuid,nodev,exec,size=268435456,mode=1770,uid=65534,gid=65534".into(),
        );
    }
    Ok(QualificationContainerSpec {
        name: format!("sy-qualification-{operation_id}"),
        image: plan.image.clone(),
        architecture: plan.target.architecture.clone(),
        entrypoint: vec![plan.operation.executable().into()],
        argv: plan
            .operation
            .arguments()
            .iter()
        ...
        image_bytes: plan.limits.image_bytes,
        pids: plan.limits.pids,
        nano_cpus: i64::from(plan.limits.cpu_cores) * 1_000_000_000,
        timeout_seconds: plan.limits.timeout_seconds,
        output_bytes: plan.limits.output_bytes,
        run_as_user: "65534:65534".into(),
        tmpfs,
    ...
    F: FnOnce(&super::engine::EngineConfig, &str, &str) -> ModelConfig,
{
    ...
    let labels = [
        ...
        ("io.sparkplane.generation", input.generation.to_string()),
        ("io.sparkplane.role", "engine".into()),
        ("io.sparkplane.model_commit", input.model_commit.clone()),
        (
            "io.sparkplane.model_repository",
            input.model_repository.clone(),
        ),
        ("io.sparkplane.engine", input.engine_id.clone()),
        (
            "io.sparkplane.engine_fingerprint",
            input.engine_fingerprint.clone(),
        ),
        (
            "io.sparkplane.artifact_fingerprint",
            input.artifact_fingerprint.clone(),
        ),
        ("io.sparkplane.operation", input.operation_id.clone()),
    ]
    .into_iter()
    .map(|(key, value)| (key.into(), value))
    .collect();
    Ok(ContainerSpec {
        name: format!("sparkplane-{}-g{}", input.instance_id, input.generation),
        image: policy.image(),
        entrypoint: config
            .entrypoint
            .iter()
...
fn verified_artifacts(artifacts: &ModelArtifactsDocument) -> Result<Vec<(&str, u64)>, ()> {
    if artifacts.schema != "sparkplane.model-artifacts/v2" || artifacts.primary.bytes == 0 {
        return Err(());
    }
...
enum ExecutorActionKind {
    ...
    RunQualification(QualificationRunInput),
...
enum QualificationRuntimeError {
    Cancelled,
    TimedOut,
    OutputLimit,
    Unavailable,
...
type QualificationRuntimeFuture =
    Pin<Box<dyn Future<Output = Result<QualificationExecution, QualificationRuntimeError>> + Send>>;
...
fn nvidia_compute_devices() -> Vec<DeviceMapping> {
    // Spark has one compute GPU. Record its nodes in the OCI configuration:
    // legacy hook-only grants disappear when Docker updates the restart policy.
    [
        "/dev/nvidia0",
        "/dev/nvidiactl",
        "/dev/nvidia-uvm",
...
#[derive(Clone)]
enum QualificationOutputChunk {
    Stdout(Vec<u8>),
    Stderr(Vec<u8>),
}

type QualificationOutputStream =
    Pin<Box<dyn futures_util::Stream<Item = Result<QualificationOutputChunk, ()>> + Send>>;

trait QualificationLifecycleApi: Send + Sync {
    fn attach(&self, container_id: String) -> RuntimeFuture<QualificationOutputStream>;
    fn start_container(&self, container_id: String) -> RuntimeFuture<()>;
    fn exit_code(&self, container_id: String) -> RuntimeFuture<i64>;
    fn cleanup(&self, container_id: String) -> RuntimeFuture<()>;
}

struct BollardQualificationLifecycle(Docker);

impl QualificationLifecycleApi for BollardQualificationLifecycle {
    fn attach(&self, container_id: String) -> RuntimeFuture<QualificationOutputStream> {
        let docker = self.0.clone();
        Box::pin(async move {
            let attached = docker
                ...
                .await
                .map_err(|_| ())?;
            let output = attached.output.map(|chunk| match chunk.map_err(|_| ())? {
                LogOutput::StdOut { message } | LogOutput::Console { message } => {
                    Ok(QualificationOutputChunk::Stdout(message.to_vec()))
                }
                LogOutput::StdErr { message } => {
                    Ok(QualificationOutputChunk::Stderr(message.to_vec()))
                }
                LogOutput::StdIn { .. } => Err(()),
            });
            Ok(Box::pin(output) as QualificationOutputStream)
        })
    }
    ...
    fn cleanup(&self, container_id: String) -> RuntimeFuture<()> {
        ...
        Box::pin(async move {
            let _ = docker
                .stop_container(
                    &container_id,
                    Some(StopContainerOptionsBuilder::default().t(1).build()),
                )
                .await;
            docker
                .remove_container(
...
async fn collect_qualification_output(
    lifecycle: &dyn QualificationLifecycleApi,
    container_id: String,
    mut output: QualificationOutputStream,
    output_limit: u64,
) -> Result<QualificationExecution, QualificationRuntimeError> {
    let output_limit =
        usize::try_from(output_limit).map_err(|_| QualificationRuntimeError::OutputLimit)?;
    let mut stdout = Vec::new();
    let mut stderr = Vec::new();
    while let Some(chunk) = output.next().await {
        let chunk = chunk.map_err(|()| QualificationRuntimeError::Unavailable)?;
        let bytes = match &chunk {
            QualificationOutputChunk::Stdout(bytes) | QualificationOutputChunk::Stderr(bytes) => {
                bytes
            }
        };
        let combined = stdout
            ...
            .ok_or(QualificationRuntimeError::OutputLimit)?;
        if combined > output_limit {
            return Err(QualificationRuntimeError::OutputLimit);
        }
        match chunk {
            QualificationOutputChunk::Stdout(bytes) => stdout.extend_from_slice(&bytes),
            QualificationOutputChunk::Stderr(bytes) => stderr.extend_from_slice(&bytes),
        }
    }
    let exit_code = lifecycle
        .exit_code(container_id)
        .await
        .map_err(|()| QualificationRuntimeError::Unavailable)?;
    QualificationExecution::from_output(&stdout, &stderr, exit_code)
        .map_err(|_| QualificationRuntimeError::OutputLimit)
}

async fn run_created_qualification(
    lifecycle: Arc<dyn QualificationLifecycleApi>,
    container_id: String,
    spec: QualificationContainerSpec,
    cancellation: CancellationToken,
) -> Result<QualificationExecution, QualificationRuntimeError> {
    let result = async {
        let attached = lifecycle
            .attach(container_id.clone())
            .await
            .map_err(|()| QualificationRuntimeError::Unavailable)?;
        if cancellation.is_cancelled() {
            return Err(QualificationRuntimeError::Cancelled);
        }
        lifecycle
            .start_container(container_id.clone())
            .await
            .map_err(|()| QualificationRuntimeError::Unavailable)?;
        let collect = collect_qualification_output(
            lifecycle.as_ref(),
            container_id.clone(),
            attached,
            spec.output_bytes,
        ...
        tokio::select! {
            biased;
            () = cancellation.cancelled() => Err(QualificationRuntimeError::Cancelled),
            result = tokio::time::timeout(Duration::from_secs(spec.timeout_seconds), collect) => {
                result.unwrap_or(Err(QualificationRuntimeError::TimedOut))
            }
        }
    }
    .await;
    let cleanup = lifecycle.cleanup(container_id).await;
    match cleanup {
        Ok(()) => result,
        Err(()) => Err(QualificationRuntimeError::Unavailable),
    }
}

fn qualification_image_architecture(host_architecture: &str) -> Result<&'static str, ()> {
    ...
    fn qualify(
        ...
        Box::pin(async move {
            let docker = Self::docker()
                .await
                .map_err(|()| QualificationRuntimeError::Unavailable)?;
            let architecture = qualification_image_architecture(&spec.architecture)
                .map_err(|()| QualificationRuntimeError::Unavailable)?;
            let image_store = BollardImageStore(docker.clone());
            let ensure = ensure_exact_image(
                &image_store,
                &spec.image,
                architecture,
            ...
            tokio::select! {
                biased;
                () = cancellation.cancelled() => return Err(QualificationRuntimeError::Cancelled),
                result = ensure => result.map_err(|()| QualificationRuntimeError::Unavailable)?,
            }
            let inspected = docker
                .inspect_image(&spec.image)
                .await
                .map_err(|_| QualificationRuntimeError::Unavailable)?;
            let size = inspected
                .size
                .and_then(|size| u64::try_from(size).ok())
                .ok_or(QualificationRuntimeError::Unavailable)?;
            if !exact_image_matches(&spec.image, architecture, &inspected)
                || size > spec.image_bytes
            {
                return Err(QualificationRuntimeError::Unavailable);
            }
            if cancellation.is_cancelled() {
                return Err(QualificationRuntimeError::Cancelled);
            }
            let body = ContainerCreateBody {
                user: Some(spec.run_as_user.clone()),
                ...
                host_config: Some(HostConfig {
                    memory: Some(
                        i64::try_from(spec.memory_bytes)
                            .map_err(|_| QualificationRuntimeError::Unavailable)?,
                    ),
                    memory_swap: Some(
                        i64::try_from(spec.memory_bytes)
                    ...
                    pids_limit: Some(spec.pids.into()),
                    ...
                    devices: Some(nvidia_compute_devices()),
                    device_requests: Some(vec![DeviceRequest {
                        driver: Some("nvidia".into()),
                        count: Some(0),
                        device_ids: Some(vec![spec.gpu_device.clone()]),
            ...
            let lifecycle: Arc<dyn QualificationLifecycleApi> =
                Arc::new(BollardQualificationLifecycle(docker));
            tokio::spawn(async move {
                let result =
                    run_created_qualification(lifecycle, created.id, spec, cancellation).await;
                let _ = tx.send(result);
            });
            rx.await
                .unwrap_or(Err(QualificationRuntimeError::Unavailable))
        })
    }
        ...
        Box::pin(async move {
            ...
            if let Ok(network) = docker.inspect_network(&name, None).await {
                ...
                if network.internal != Some(true)
                    ...
                    || labels.get("io.sparkplane.managed").map(String::as_str) != Some("true")
                {
                    return Err(());
                }
            ...
            let labels = std::collections::HashMap::from([
                ("io.sparkplane.managed".into(), "true".into()),
                ("io.sparkplane.role".into(), "network".into()),
            ]);
            docker
        ...
        Box::pin(async move {
            ...
            let body = ContainerCreateBody {
                ...
                host_config: Some(HostConfig {
                    ...
                    devices: match spec.accelerator {
                        Accelerator::Cpu => None,
                        Accelerator::Nvidia => Some(nvidia_compute_devices()),
                    },
        ...
        Box::pin(async move {
            ...
            let filters: std::collections::HashMap<String, Vec<String>> =
                std::collections::HashMap::from([(
                    "label".into(),
                    vec!["io.sparkplane.managed=true".into()],
                )]);
            let listed = docker
    ...
    fn quarantine(&self, container_id: String) -> RuntimeFuture<()> {
        Box::pin(async move {
            ...
            let managed = inspect
                ...
                .and_then(|labels| labels.get("io.sparkplane.managed"))
                .map(String::as_str);
...
fn managed_observation(
    ...
    Ok(ManagedContainerObservation {
        ...
        name: inspect
            ...
            .trim_start_matches('/')
            .into(),
        instance_id: labels.get("io.sparkplane.instance").cloned(),
        generation: labels
            .get("io.sparkplane.generation")
            .and_then(|value| value.parse().ok()),
        role: labels.get("io.sparkplane.role").cloned(),
        model_commit: labels.get("io.sparkplane.model_commit").cloned(),
        model_repository: labels.get("io.sparkplane.model_repository").cloned(),
        engine_id: labels.get("io.sparkplane.engine").cloned(),
        engine_fingerprint: labels.get("io.sparkplane.engine_fingerprint").cloned(),
        artifact_fingerprint: labels.get("io.sparkplane.artifact_fingerprint").cloned(),
...
async fn inspect_input(
    ...
    let name = format!("sparkplane-{}-g{}", input.instance_id, input.generation);
...
fn exact_managed_identity(
    ...
    labels.get("io.sparkplane.managed").map(String::as_str) == Some("true")
        && labels.get("io.sparkplane.role").map(String::as_str) == Some("engine")
        && labels.get("io.sparkplane.instance").map(String::as_str)
            == Some(input.instance_id.as_str())
        && labels.get("io.sparkplane.generation").map(String::as_str)
...
fn observed_from_inspect(
    ...
    let labels = inspect
        ...
        .and_then(|config| config.labels.as_ref())
        .ok_or(())?;
    let instance_id = labels.get("io.sparkplane.instance").cloned().ok_or(())?;
...
impl DockerInspector for BollardDockerInspector {
    fn inspect(&self, cancellation: CancellationToken) -> DockerFuture {
        Box::pin(async move {
            ...
            Ok(DockerCapability {
                schema: "sparkplane.executor.docker/v1".into(),
                transport: "unix".into(),
...
struct QualificationExecutorConfig {
    authority: String,
    target: QualificationTarget,
    slot: Arc<tokio::sync::Semaphore>,
    active: Mutex<BTreeMap<Ulid, Arc<tokio::sync::Semaphore>>>,
}

fn register_qualification_request<F>(
    active: &Mutex<BTreeMap<Ulid, Arc<tokio::sync::Semaphore>>>,
    cancellation: &CancelRegistry,
    request_id: Ulid,
    completed: Arc<tokio::sync::Semaphore>,
    after_register: F,
) -> sparkplane_ipc::CancelGuard
where
    F: FnOnce(),
{
    ...
    let guard = cancellation.register(request_id);
    after_register();
    active.insert(request_id, completed);
    guard
}
...
impl ExecutorHandler {
    fn production(
        ...
        let authority = std::fs::read_to_string(RELEASE_PUBLIC_KEY_PATH)
            .map_err(|error| format!("read qualification release authority: {error}"))?;
        Ok(Self {
            ...
            qualification: Some(Arc::new(QualificationExecutorConfig {
                authority,
                target: QualificationTarget {
                    architecture: config.host.architecture.clone(),
                    gpu_model: config.host.gpu_model.clone(),
                    compute_capability: config.host.compute_capability.clone(),
                    dgx_build: config.host.dgx_build.clone(),
                    driver_version: config.host.driver_version.clone(),
                    toolkit_version: config.host.toolkit_version.clone(),
                    protected_fingerprint: config.host.protected_fingerprint.clone(),
                },
    ...
    ) -> Result<QualificationPlanDocument, ErrorCode> {
        let qualification = self.qualification.as_ref().ok_or(ErrorCode::NotReady)?;
        let mut verified = super::qualification::verify_submission(
            submission,
            &qualification.authority,
            &qualification.target,
        )
        .map_err(|_| ErrorCode::BadRequest)?;
        let (snapshot, policy) = self
            .resources
            .as_ref()
            .and_then(|resources| resources.snapshot())
            .ok_or(ErrorCode::NotReady)?;
        super::qualification::admit_resources(
            &mut verified.plan,
            &snapshot,
            &policy,
            super::resources::unix_millis(),
        )
        .map_err(|_| ErrorCode::NotReady)?;
        Ok(verified.plan)
    }
    ...
    async fn execute(
        ...
        match action.action {
            ...
            ExecutorActionKind::InspectEmergencyRecords => self
                ...
                .ok_or(ErrorCode::NotReady),
            ExecutorActionKind::InspectQualification(submission) => self
                .qualification_plan(&submission)
                .map(|plan| ExecutorResult::InspectQualification { plan }),
            ExecutorActionKind::RunQualification(input) => {
                let plan = self.qualification_plan(&input.submission)?;
                let qualification = self.qualification.as_ref().ok_or(ErrorCode::NotReady)?;
                let _slot = qualification
                    .slot
                    .clone()
                    .try_acquire_owned()
                    .map_err(|_| ErrorCode::Overloaded)?;
                let spec = build_qualification_container_spec(&plan, &input.operation_id)
                    .map_err(|()| ErrorCode::BadRequest)?;
                let execution = self.runtime.qualify(spec, cancellation).await.map_err(
                    |error| match error {
                        QualificationRuntimeError::Cancelled => ErrorCode::Cancelled,
                        QualificationRuntimeError::TimedOut => ErrorCode::Timeout,
                        QualificationRuntimeError::OutputLimit => ErrorCode::PolicyDenied,
                        QualificationRuntimeError::Unavailable => ErrorCode::NotReady,
                    },
                )?;
                Ok(ExecutorResult::RunQualification { plan, execution })
            }
    ...
    fn inspect_host(&self) -> Result<ProtectedHostSnapshot, ()> {
        ...
        Ok(ProtectedHostSnapshot {
            schema: "sparkplane.executor.host/v1".into(),
            hostname,
            kernel_release,
    ...
    async fn dispatch(&self, req: Request) -> Response {
        ...
        if matches!(
            &action.action,
            ExecutorActionKind::RunQualification(input)
                if Ulid::from_string(&input.operation_id).ok() != Some(req.request_id)
        ) {
            return error_response(
                req.request_id,
                ErrorCode::BadRequest,
                "qualification operation identity mismatch",
            );
        }
        ...
        if matches!(&action.action, ExecutorActionKind::RunQualification(_)) {
            let completed = Arc::new(tokio::sync::Semaphore::new(0));
            let guard = if let Some(qualification) = &self.qualification {
                register_qualification_request(
                ...
                self.cancellation.register(req.request_id)
            };
            let cancellation = guard.token();
            let result = await_qualification_completion(
                self.execute(action, cancellation.clone()),
                cancellation,
                deadline,
            )
            .await;
            ...
            return match result {
                ...
                Err(code) => executor_error(req.request_id, code),
            };
        }
        let guard = self.cancellation.register(req.request_id);
        let timeout_cancellation = guard.token();
    ...
    async fn cancel(&self, req: &Request) -> Response {
        #[derive(Deserialize)]
        #[serde(deny_unknown_fields)]
        struct CancelParams {
        ...
        match serde_json::from_value::<CancelParams>(req.params.clone()) {
            Ok(params) => {
                let completion = self.qualification.as_ref().and_then(|qualification| {
                    qualification
                        .active
                        .lock()
                        .unwrap_or_else(std::sync::PoisonError::into_inner)
                ...
                let cancelled = self.cancellation.cancel(params.target_request_id);
                if cancelled
                    && let Some(completion) = completion
                    && tokio::time::timeout(
                        Duration::from_secs(QUALIFICATION_CLEANUP_ACK_SECONDS),
                        completion.acquire(),
                    )
                    .await
                    .is_err()
                {
                    return error_response(
                        req.request_id,
                        ErrorCode::Internal,
                        "qualification cleanup acknowledgement failed",
                    );
                }
                ok_response(
                    req.request_id,
...
async fn await_qualification_completion<F>(
    execution: F,
    cancellation: CancellationToken,
    deadline: Duration,
) -> Result<ExecutorResult, ErrorCode>
where
    F: Future<Output = Result<ExecutorResult, ErrorCode>>,
{
    tokio::pin!(execution);
    tokio::select! {
...
impl ExecutorClient {
    ...
    pub async fn qualification_plan(
        &self,
        submission: QualificationSubmission,
    ) -> Result<QualificationPlanDocument, ExecutorClientError> {
        match self
            .call(ExecutorAction {
                action: ExecutorActionKind::InspectQualification(submission),
            })
            .await?
        {
            ExecutorResult::InspectQualification { plan } => Ok(plan),
            _ => Err(protocol_error()),
        }
    }
    ...
    pub async fn run_qualification(
        ...
        deadline: Duration,
    ) -> Result<(QualificationPlanDocument, QualificationExecution), ExecutorClientError> {
        let request_id = Ulid::from_string(&input.operation_id).map_err(|_| protocol_error())?;
        if deadline.is_zero() || deadline.as_millis() > u128::from(MAX_DEADLINE_MS) {
            return Err(protocol_error());
        }
        match self
            .call_with_request_id_and_response_timeout(
                ExecutorAction {
                    action: ExecutorActionKind::RunQualification(input),
                },
                deadline,
                qualification_response_timeout(deadline),
                Some(request_id),
            )
            .await?
        {
            ExecutorResult::RunQualification { plan, execution } => Ok((plan, execution)),
            _ => Err(protocol_error()),
        }
    }

    pub async fn cancel_qualification(
        &self,
        operation_id: Ulid,
    ) -> Result<bool, ExecutorClientError> {
        let response_timeout = qualification_response_timeout(Duration::ZERO);
        let mut client =
            tokio::time::timeout(self.deadline, sparkplane_ipc::Client::connect(&self.socket))
                .await
        ...
        let response = tokio::time::timeout(
            ...
            client.call(
                "system.cancel",
                serde_json::json!({"target_request_id": operation_id}),
                CallOpts {
                    priority: Priority::Interactive,
                    deadline_ms: Some(response_timeout.as_millis() as u64),
                    ..CallOpts::default()
                },
            ),
        )
        ...
        match response {
            ...
            Response::Err { error, .. } => Err(ExecutorClientError {
                code: error.code,
                detail: "executor rejected qualification cancellation",
            }),
        }
    }
    ...
    async fn call_with_timeout(
        ...
        self.call_with_request_id(action, deadline, None).await
    }
    ...
    async fn call_with_request_id(
        ...
        self.call_with_request_id_and_response_timeout(action, deadline, deadline, request_id)
            .await
    }

    async fn call_with_request_id_and_response_timeout(
        &self,
        action: ExecutorAction,
        ...
        let mut client = tokio::time::timeout(
            self.deadline.min(response_timeout),
            sparkplane_ipc::Client::connect(&self.socket),
        )
        .await
        .map_err(|_| unavailable_error())?
        .map_err(|_| unavailable_error())?;
        let response = tokio::time::timeout(
            ...
            client.call(
                ...
                CallOpts {
                    priority: Priority::Interactive,
                    deadline_ms: Some(request_deadline.as_millis() as u64),
                    request_id,
                    ..CallOpts::default()
                },
            ),
...
pub async fn serve(config_path: &Path) -> anyhow::Result<()> {
    ...
    anyhow::ensure!(
        config.schema == "sparkplane.executor/v1",
    ...
    let resources = Arc::new(ResourceMonitor::new(
        ...
        resource_policy,
        heartbeats.clone(),
        "/var/lib/sparkplane/executor/emergency.jsonl".into(),
    ));
...
fn managed_event_filters() -> std::collections::HashMap<String, Vec<String>> {
    std::collections::HashMap::from([
        ...
        ("label".into(), vec!["io.sparkplane.managed=true".into()]),
    ])
}
...
fn managed_event_is_relevant(event: &EventMessage) -> bool {
    ...
    let instance = attributes.get("io.sparkplane.instance");
    valid_id
        && bounded
        && attributes.get("io.sparkplane.managed").map(String::as_str) == Some("true")
        && attributes.get("io.sparkplane.role").map(String::as_str) == Some("engine")
...
async fn resource_sampling_loop(
    ...
    loop {
        ...
        if let Some(decision) = resources.take_emergency()
            && actuate_emergency(&resources, runtime.as_ref(), &decision).await
        {
...
mod tests {
    ...
    fn compute_devices_survive_restart_policy_updates() {
        let paths: Vec<_> = nvidia_compute_devices()
            .into_iter()
            .map(|device| {
                assert_eq!(device.path_in_container, device.path_on_host);
                assert_eq!(device.cgroup_permissions.as_deref(), Some("rw"));
                device.path_on_host.unwrap()
            })
            .collect();
        assert_eq!(
            paths,
    ...
    use crate::spark::qualification::{
        QualificationIsolation, QualificationLimits, QualificationOperation,
        QualificationResourceAdmission,
    };

    #[test]
    fn qualification_client_wait_includes_cleanup_acknowledgement() {
        assert!(
            qualification_response_timeout(Duration::from_millis(5))
                > Duration::from_secs(QUALIFICATION_CLEANUP_ACK_SECONDS)
        );
    }

    struct FakeQualificationLifecycle {
        attach_ok: bool,
        start_ok: bool,
        cleanup_ok: bool,
        pending_output: bool,
        output: Vec<QualificationOutputChunk>,
        cleanup_calls: Arc<AtomicU64>,
        cleanup_gate: Option<Arc<tokio::sync::Semaphore>>,
    }

    impl QualificationLifecycleApi for FakeQualificationLifecycle {
        fn attach(&self, _: String) -> RuntimeFuture<QualificationOutputStream> {
            ...
            Box::pin(async move {
                ...
                if pending {
                    Ok(Box::pin(futures_util::stream::pending()) as QualificationOutputStream)
                } else {
                    Ok(
                        Box::pin(futures_util::stream::iter(output.into_iter().map(Ok)))
                            as QualificationOutputStream,
                    )
                }
            })
        }

        fn start_container(&self, _: String) -> RuntimeFuture<()> {
            let start_ok = self.start_ok;
            Box::pin(async move { start_ok.then_some(()).ok_or(()) })
        }

        fn exit_code(&self, _: String) -> RuntimeFuture<i64> {
            Box::pin(async { Ok(0) })
        }

        fn cleanup(&self, _: String) -> RuntimeFuture<()> {
            self.cleanup_calls.fetch_add(1, Ordering::SeqCst);
            let cleanup_ok = self.cleanup_ok;
            let gate = self.cleanup_gate.clone();
            Box::pin(async move {
                if let Some(gate) = gate {
                    let _permit = gate.acquire().await.map_err(|_| ())?;
                }
                cleanup_ok.then_some(()).ok_or(())
            })
        }
    }

    fn fake_qualification_lifecycle(
        attach_ok: bool,
        start_ok: bool,
    ) -> Arc<FakeQualificationLifecycle> {
        Arc::new(FakeQualificationLifecycle {
            attach_ok,
            start_ok,
            cleanup_ok: true,
    ...
    #[test]
    fn qualification_spec_is_one_shot_isolated_and_unmanaged() {
        let plan = qualification_plan(8 * 1024 * 1024 * 1024);

        let spec = build_qualification_container_spec(&plan, "01J00000000000000000000000")
            .expect("valid qualification spec");

        assert_eq!(spec.gpu_device, "0");
        assert_eq!(spec.network, "none");
        assert!(spec.read_only_rootfs);
        assert!(!spec.privileged);
        assert_eq!(spec.cap_drop, ["ALL"]);
        assert_eq!(spec.restart, "no");
        assert_eq!(spec.run_as_user, "65534:65534");
        assert!(spec.tmpfs["/tmp"].contains("noexec"));
        assert_eq!(spec.entrypoint, [plan.operation.executable()]);
        assert_eq!(spec.argv, plan.operation.arguments());
        assert!(
            spec.labels
                .keys()
                .all(|label| label.starts_with("io.sparkplane.qualification"))
        );
        for managed in [
            "io.sparkplane.managed",
            "io.sparkplane.role",
            "io.sparkplane.instance",
            "io.sparkplane.generation",
            "io.sparkplane.engine",
            "io.sparkplane.model_commit",
        ] {
            assert!(!spec.labels.contains_key(managed));
        }
    }

    #[test]
    fn qsa_qualification_spec_cannot_change_the_fixed_command() {
        let mut plan = qualification_plan(8 * 1024 * 1024 * 1024);
        plan.operation = QualificationOperation::SparkFlashQsaV1;
        let spec = build_qualification_container_spec(&plan, "01J00000000000000000000000").unwrap();
        assert_eq!(spec.entrypoint, ["/opt/sparky/bin/sparky-qualification"]);
        assert_eq!(spec.argv, ["qsa-target", "--json"]);
        assert!(spec.tmpfs["/tmp"].contains("noexec"));
        assert!(spec.tmpfs["/tmp/triton"].contains("exec"));
        assert!(!spec.tmpfs["/tmp/triton"].contains("noexec"));
        assert!(
            spec.environment
                .contains(&"TRITON_CACHE_DIR=/tmp/triton".into())
        );
    }

    #[test]
    fn moe_qualification_retains_bounded_qsa_jit_isolation() {
        let mut plan = qualification_plan(8 * 1024 * 1024 * 1024);
        plan.operation = QualificationOperation::SparkFlashQsaV1;
        let mut qsa =
            build_qualification_container_spec(&plan, "01J00000000000000000000000").unwrap();
        plan.operation = QualificationOperation::SparkFlashMoeV1;
        let moe = build_qualification_container_spec(&plan, "01J00000000000000000000000").unwrap();
        assert_eq!(moe.argv, ["moe-target", "--json"]);
        qsa.argv = moe.argv.clone();
        assert_eq!(moe, qsa);
    }

    #[test]
    fn qualification_slot_allows_only_one_gpu_job() {
    ...
    #[test]
    fn lifecycle_qualification_retains_precompiled_isolation() {
        let mut plan = qualification_plan(8 * 1024 * 1024 * 1024);
        let mut expected =
            build_qualification_container_spec(&plan, "00000000000000000000000000").unwrap();
        plan.operation = QualificationOperation::SparkFlashCudaLifecycleV1;
        let actual =
            build_qualification_container_spec(&plan, "00000000000000000000000000").unwrap();
        expected.argv = vec!["cuda-lifecycle-target".into(), "--json".into()];
        assert_eq!(actual, expected);
    }

    #[test]
    fn memory_qualification_retains_precompiled_isolation() {
        let mut plan = qualification_plan(16 * 1024 * 1024 * 1024);
        let mut expected =
            build_qualification_container_spec(&plan, "00000000000000000000000000").unwrap();
        plan.operation = QualificationOperation::SparkFlashMemoryAdmissionV1;
        let actual =
            build_qualification_container_spec(&plan, "00000000000000000000000000").unwrap();
        expected.argv = vec!["memory-admission-target".into(), "--json".into()];
        assert_eq!(actual, expected);
    }

    #[test]
    fn qualification_graph_retains_precompiled_isolation() {
        let mut plan = qualification_plan(24 * 1024 * 1024 * 1024);
        let mut expected =
            build_qualification_container_spec(&plan, "00000000000000000000000000").unwrap();
        plan.operation = QualificationOperation::SparkFlashGraphCatalogV1;
        let actual =
            build_qualification_container_spec(&plan, "00000000000000000000000000").unwrap();
        expected.argv = vec!["graph-catalog-target".into(), "--json".into()];
        assert_eq!(actual, expected);
    }

    #[test]
    fn catalog_qualification_retains_precompiled_no_jit_isolation() {
        let mut plan = qualification_plan(8 * 1024 * 1024 * 1024);
        let mut expected =
            build_qualification_container_spec(&plan, "01J00000000000000000000000").unwrap();
        plan.operation = QualificationOperation::SparkFlashKernelCatalogV1;
        let actual =
            build_qualification_container_spec(&plan, "01J00000000000000000000000").unwrap();
        expected.argv = vec!["kernel-catalog-target".into(), "--json".into()];
        assert_eq!(actual, expected);
        assert_eq!(actual.tmpfs.len(), 1);
        assert!(
            !actual
                .environment
                .iter()
                .any(|value| value.starts_with("TRITON_"))
        );
    }

    #[test]
    fn qualification_session_retains_precompiled_isolation() {
        let mut plan = qualification_plan(24 * 1024 * 1024 * 1024);
        let mut expected =
            build_qualification_container_spec(&plan, "00000000000000000000000000").unwrap();
        plan.operation = QualificationOperation::SparkFlashSessionV1;
        let actual =
            build_qualification_container_spec(&plan, "00000000000000000000000000").unwrap();
        expected.argv = vec!["session-target".into(), "--json".into()];
        assert_eq!(actual, expected);
    }

    #[test]
    fn qualification_model_prefix_retains_precompiled_isolation() {
        let mut plan = qualification_plan(24 * 1024 * 1024 * 1024);
        let id = "00000000000000000000000000";
        let mut expected = build_qualification_container_spec(&plan, id).unwrap();
        plan.operation = QualificationOperation::SparkFlashModelPrefixV1;
        let actual = build_qualification_container_spec(&plan, id).unwrap();
        expected.argv = vec!["model-prefix-target".into(), "--json".into()];
        assert_eq!(actual, expected);
    }

    #[tokio::test]
    async fn qualification_cancellation_waits_for_cleanup_acknowledgement() {
        ...
        let completion = tokio::spawn(await_qualification_completion(
            async move {
                worker_token.cancelled().await;
                worker_observed.notify_one();
                let _permit = worker_cleanup.acquire().await.unwrap();
        ...
        assert_eq!(completion.await.unwrap(), Err(ErrorCode::Cancelled));
    }

    #[test]
    fn qualification_registration_is_atomic_against_cancel_lookup() {
        ...
        let registration_active = active.clone();
        let registration_cancellation = cancellation.clone();
        let registration = std::thread::spawn(move || {
            let guard = register_qualification_request(
                &registration_active,
        ...
        assert!(active.try_lock().is_err());
        let cancel_active = active.clone();
        let cancel_registry = cancellation.clone();
        let (cancel_tx, cancel_rx) = std::sync::mpsc::channel();
        let cancel = std::thread::spawn(move || {
            let completion = cancel_active.lock().unwrap().get(&request_id).cloned();
            cancel_tx
                .send((completion.is_some(), cancel_registry.cancel(request_id)))
                .unwrap();
        });

        continue_tx.send(()).unwrap();
        let _guard = guard_rx.recv().unwrap();
        assert_eq!(cancel_rx.recv().unwrap(), (true, true));
        registration.join().unwrap();
        cancel.join().unwrap();
    }

    #[tokio::test]
    async fn qualification_created_container_cleanup_covers_every_terminal_path() {
        for (attach_ok, start_ok, cancellation) in [
            (false, true, CancellationToken::new()),
            (true, false, CancellationToken::new()),
            (true, true, {
            ...
            let lifecycle = fake_qualification_lifecycle(attach_ok, start_ok);
            let calls = lifecycle.cleanup_calls.clone();
            let result = run_created_qualification(
                lifecycle,
                "exact-container-id".into(),
                build_qualification_container_spec(
                    &qualification_plan(8 * 1024 * 1024 * 1024),
    ...
    #[tokio::test]
    async fn qualification_cleanup_failure_cannot_report_passed() {
        let lifecycle = Arc::new(FakeQualificationLifecycle {
            cleanup_ok: false,
            ..Arc::try_unwrap(fake_qualification_lifecycle(true, true))
                .ok()
                .unwrap()
        });
        ...
        assert_eq!(result, Err(QualificationRuntimeError::Unavailable));
    ...
    #[tokio::test]
    async fn qualification_output_overflow_cleans_the_exact_container() {
        let lifecycle = Arc::new(FakeQualificationLifecycle {
            output: vec![QualificationOutputChunk::Stdout(vec![0; 1025])],
            ..Arc::try_unwrap(fake_qualification_lifecycle(true, true))
                .ok()
                .unwrap()
        });
        ...
        assert_eq!(result, Err(QualificationRuntimeError::OutputLimit));
        assert_eq!(calls.load(Ordering::SeqCst), 1);
    }

    #[tokio::test]
    async fn qualification_explicit_cancel_does_not_finish_before_cleanup() {
        ...
        let calls = lifecycle.cleanup_calls.clone();
        let cancellation = CancellationToken::new();
        let mut spec = build_qualification_container_spec(
            &qualification_plan(8 * 1024 * 1024 * 1024),
            "01J00000000000000000000000",
        )
        .unwrap();
        spec.timeout_seconds = 60;
        let run = tokio::spawn(run_created_qualification(
            lifecycle,
            "exact-container-id".into(),
            spec,
            cancellation.clone(),
        ...
        assert_eq!(
            run.await.unwrap(),
            Err(QualificationRuntimeError::Cancelled)
        );
    }

    #[tokio::test]
    async fn qualification_timeout_cleans_before_returning() {
        let lifecycle = Arc::new(FakeQualificationLifecycle {
            pending_output: true,
            ..Arc::try_unwrap(fake_qualification_lifecycle(true, true))
                .ok()
        ...
        assert_eq!(
            ...
            .await,
            Err(QualificationRuntimeError::TimedOut)
        );
        assert_eq!(calls.load(Ordering::SeqCst), 1);
    }

    fn qualification_plan(memory_bytes: u64) -> QualificationPlanDocument {
        QualificationPlanDocument {
            ...
            resources: Some(QualificationResourceAdmission {
                observed_at_unix_ms: 10_000,
                mem_available_bytes: 20 * 1024 * 1024 * 1024,
                live_available_after_start_bytes: 12 * 1024 * 1024 * 1024,
                required_available_floor_bytes: 8 * 1024 * 1024 * 1024,
                disk_available_bytes: 200 * 1024 * 1024 * 1024,
                disk_available_after_pull_bytes: 200 * 1024 * 1024 * 1024 - 1024,
                required_disk_reserve_bytes: 100 * 1024 * 1024 * 1024,
        ...
        fn qualify(
            ...
            Box::pin(async move {
                if cancellation.is_cancelled() {
                    ...
                    QualificationExecution::from_output(b"fixture ok\n", b"", 0)
                        .map_err(|_| QualificationRuntimeError::OutputLimit)
                }
            })
        }
        ...
        let inspect = bollard::models::ContainerInspectResponse {
            ...
            config: Some(bollard::models::ContainerConfig {
                labels: Some(std::collections::HashMap::from([
                    ("io.sparkplane.managed".into(), "true".into()),
                    ("io.sparkplane.role".into(), "engine".into()),
                    ("io.sparkplane.instance".into(), input.instance_id.clone()),
                    ("io.sparkplane.generation".into(), "1".into()),
                ])),
                ..Default::default()
            }),
    ...
    fn unrelated_or_malformed_docker_events_never_wake_reconciliation() {
        ...
        malformed
            ...
            .as_mut()
            .unwrap()
            .insert("io.sparkplane.generation".into(), "0".into());
    ...
    fn vllm_policy() -> Arc<EnginePolicy> {
        Arc::new(
            EnginePolicy::parse(include_str!("../../configs/sparkplane/engines/vllm.toml"))
                .unwrap(),
        )
    }
    ...
    fn engine_catalog() -> Arc<EngineCatalog> {
        Arc::new(
            EngineCatalog::parse_files([
                (
                    "llama-cpp.toml",
                    include_str!("../../configs/sparkplane/engines/llama-cpp.toml"),
                ),
                (
                    "vllm.toml",
                    include_str!("../../configs/sparkplane/engines/vllm.toml"),
                ),
            ])
            .unwrap(),
        )
    ...
    fn test_artifacts() -> ModelArtifactsDocument {
        serde_json::from_str(r#"{"schema":"sparkplane.model-artifacts/v2","format":"safetensors","primary":{"path":"model.safetensors","bytes":8,"sha256":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"},"auxiliary":[],"quantization":"FP8","capabilities":["text_generation"],"configured_alias":null}"#).unwrap()
    }

    fn gguf_artifacts() -> ModelArtifactsDocument {
        serde_json::from_str(r#"{"schema":"sparkplane.model-artifacts/v2","format":"gguf","primary":{"path":"weights/model.gguf","bytes":8,"sha256":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"},"auxiliary":[{"role":"projector","path":"vision/mmproj.gguf","bytes":4,"sha256":"bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"},{"role":"weight_shard","path":"weights/model-00002.gguf","bytes":3,"sha256":"cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc"}],"quantization":"Q4_K_XL","capabilities":["text_generation"],"configured_alias":"model:q4"}"#).unwrap()
    }
    ...
    fn signed_executor_config_pins_catalog_and_exact_host_identity() {
        let config: ExecutorConfig =
            toml::from_str(include_str!("../../configs/sparkplane/executor.toml")).unwrap();
    ...
    fn unknown_action_field_or_oversized_frame_fails_closed() {
        ...
        let error = sparkplane_ipc::RequestCodec::with_max_frame_length(MAX_REQUEST_FRAME)
            .decode(&mut frame)
    ...
    fn configured_model_prepare_request_fits_executor_frame() {
        ...
        let models = super::super::model_catalog::ModelCatalog::parse(include_str!(
            "../../configs/sparkplane/models.toml"
        ))
        .unwrap();
    ...
    fn container_ipc_and_shared_memory_come_from_engine_configuration() {
        let configured = include_str!("../../configs/sparkplane/engines/vllm.toml")
    ...
    fn container_limit_uses_the_selected_profile_resource_envelope() {
        let configured = include_str!("../../configs/sparkplane/engines/llama-cpp.toml").replacen(
            "id = \"qwen3.8-mtp\"",
    ...
    fn scheduling_metadata_preserves_compile_cache_namespace() {
        ...
        let changed = include_str!("../../configs/sparkplane/engines/llama-cpp.toml").replacen(
            "priority = 100",
            "priority = 101",
            1,
        );
    ...
    async fn emergency_disables_restart_then_kills_exact_observed_cgroup() {
        ...
        let decision = super::super::resources::EmergencyDecision {
            schema: "sparkplane.emergency-decision/v1".into(),
            instance_id: observed.instance_id.clone(),
            generation: observed.generation,
    ...
    fn stop_target_requires_exact_managed_identity() {
        ...
        let mut labels = std::collections::HashMap::from([
            ...
            ("io.sparkplane.role".into(), "engine".into()),
            ("io.sparkplane.instance".into(), input.instance_id.clone()),
            ("io.sparkplane.generation".into(), "7".into()),
        ]);

        assert!(exact_managed_identity(&labels, &input));
        labels.insert("io.sparkplane.generation".into(), "6".into());
        assert!(!exact_managed_identity(&labels, &input));
    }
