...
use super::wire::{
    RECIPE_CATALOG_SCHEMA, RecipeCatalogDocument, RecipeCompatibilityDocument,
    RecipeEvidenceDocument, RecipeMismatchDocument, RecipeResourceEnvelopeDocument,
    RecipeSelectionDocument, RecipeSelectionReason, RecipeStatus,
};

const RECIPE_SCHEMA: &str = "sparkplane.recipe/v1";
const CATALOG_SCHEMA: &str = "sparkplane.recipe-catalog/v1";
...
const SIGNED_RECIPES: [(&str, &[u8]); 3] = [
    (
        ORNITH_FILE,
        include_bytes!("../../configs/sparkplane/recipes/ornith-vllm.toml"),
    ),
    (
        QWEN_FILE,
        include_bytes!("../../configs/sparkplane/recipes/qwen3-embedding.toml"),
    ),
    (
        FIXTURE_FILE,
        include_bytes!("../../configs/sparkplane/recipes/fixture-http-echo.toml"),
    ),
];
...
fn validate_recipe(recipe: &Recipe) -> Result<(), String> {
    ...
    if recipe.isolation.network_policy != "sparkplane-internal-v1"
...
fn select(
    ...
    if let Some(winner) = &request.tuned_winner
        && winner.expires_at > request.now
        && let Some(candidate) = compatibility
            .iter()
...
mod tests {
    ...
    fn strict_schema_rejects_every_unbounded_or_mutable_field() {
        ...
        for altered in [
            ...
            original.replace(
                "host_root = \"/var/lib/sparkplane/huggingface\"",
    ...
    fn full_fingerprint_changes_on_every_identity_input() {
        ...
        let mut changed = recipe.clone();
        changed.evidence.corpus = "sparkplane.agent-corpus/v2".into();
        assert_ne!(
            recipe_fingerprint(&changed, &host(), "agent").unwrap(),
            baseline
        );
