...
use super::wire::{
    INSTANCE_SCHEMA, InstanceDesiredState, InstanceDocument, InstanceObservedState, ModelDocument,
    OPERATION_EVENT_SCHEMA, OPERATION_SCHEMA, OperationDocument, OperationEvent, OperationProgress,
    OperationState, PROBLEM_SCHEMA, ProblemDocument, TOKEN_SCHEMA, TokenCreateRequest,
    TokenDocument,
};
...
fn import_emergency(
    ...
    if record.schema != "sparkplane.emergency-record/v1"
    ...
    for operation_id in active {
        ...
        transition(
            ...
            Some(ProblemDocument {
                ...
                r#type: "https://sparkplane.local/problems/spark-memory-emergency".into(),
...
mod tests {
    ...
    fn instance_document(model: &ModelDocument, name: &str, recipe: &str) -> InstanceDocument {
        let artifacts = serde_json::from_str(r#"{"schema":"sparkplane.model-artifacts/v2","format":"safetensors","primary":{"path":"model.safetensors","bytes":8,"sha256":null},"auxiliary":[],"quantization":"FP8","capabilities":["text_generation"],"configured_alias":null}"#).unwrap();
        InstanceDocument {
    ...
    async fn token_snapshot_verifies_hmac_without_plaintext_database_storage() {
        ...
        assert!(
            !fs::read(root.path().join("state.sqlite3"))
                .unwrap()
                .windows(secret.len())
    ...
    async fn artifact_model_metadata_round_trips_exactly() {
        ...
        const JSON: &str = r#"{"schema":"sparkplane.model-artifacts/v2","format":"gguf","primary":{"path":"model.gguf","bytes":7,"sha256":"aaa"},"auxiliary":[{"role":"projector","path":"mmproj.gguf","bytes":3,"sha256":"bbb"}],"quantization":"Q4_K_XL","capabilities":["text","vision"],"configured_alias":"artifact"}"#;
