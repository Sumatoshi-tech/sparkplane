...
fn build_claude_code_config(
    ...
    ClaudeCodeClientConfig {
        schema: "sparkplane.client-config/v1".into(),
...
fn build_codex_config(
    ...
    const ENV_KEY: &str = "SPARKPLANE_INFERENCE_TOKEN";
    let provider = format!("sparkplane_{}", instance.replace(['-', '.'], "_"));
    let toml = format!(
        "model = {model:?}\nmodel_provider = {provider:?}\nweb_search = \"disabled\"\n\n[model_providers.{provider}]\nname = {name:?}\nbase_url = {base_url:?}\nenv_key = {env:?}\nwire_api = \"responses\"\nsupports_standalone_web_search = false\nsupports_websockets = false\n",
        name = format!("Sparkplane {instance}"),
        env = ENV_KEY
    );
    CodexClientConfig {
        schema: "sparkplane.client-config/v1".into(),
...
impl SparkClient {
    ...
    pub fn list_operations(&self) -> Result<OperationListDocument, ClientError> {
        self.get_json("api/sparkplane/v1/operations")
    }

    pub fn qualification_plan(
        &self,
        key: &str,
        request: &super::qualification::QualificationSubmission,
    ) -> Result<super::qualification::QualificationPlanDocument, ClientError> {
        self.mutation_json(
            Method::POST,
            "api/sparkplane/v1/qualifications",
            key,
            Some(request),
        )
    }
    ...
    pub fn list_models(&self) -> Result<ModelListDocument, ClientError> {
        self.get_json("api/sparkplane/v1/models")
    }
    ...
    pub fn model(&self, id: &str) -> Result<ModelDocument, ClientError> {
        ...
        self.get_json(&format!("api/sparkplane/v1/models/{id}"))
    }
    ...
    pub fn instances(&self) -> Result<InstanceListDocument, ClientError> {
        self.get_json("api/sparkplane/v1/instances")
    }
    ...
    pub fn stop_instance(
        ...
        self.mutation_json(
            Method::DELETE,
            &format!("api/sparkplane/v1/instances/{id}"),
            key,
            Some(request),
        )
    }
    ...
    pub fn instance_logs(
        ...
        self.get_json(&format!(
            "api/sparkplane/v1/instances/{id}/logs?cursor={cursor}&limit={limit}"
        ))
    }
    ...
    pub fn removal_plan(
        ...
        self.mutation_json(
            Method::DELETE,
            &format!("api/sparkplane/v1/models/{id}"),
            key,
            Some(request),
        )
    }
    ...
    pub fn operation(&self, id: &str) -> Result<OperationDocument, ClientError> {
        validate_id(id)?;
        self.get_json(&format!("api/sparkplane/v1/operations/{id}"))
    }
    ...
    pub fn cancel_operation(&self, id: &str, key: &str) -> Result<OperationDocument, ClientError> {
        ...
        self.mutation_json::<(), _>(
            Method::DELETE,
            &format!("api/sparkplane/v1/operations/{id}"),
            key,
            None,
        )
    }
    ...
    pub fn create_token(
        ...
        self.mutation_json(Method::POST, "api/sparkplane/v1/tokens", key, Some(request))
    }

    pub fn list_tokens(&self) -> Result<TokenListDocument, ClientError> {
        self.get_json("api/sparkplane/v1/tokens")
    }
    ...
    pub fn revoke_token(&self, id: &str, key: &str) -> Result<OperationDocument, ClientError> {
        ...
        self.mutation_json::<(), _>(
            Method::DELETE,
            &format!("api/sparkplane/v1/tokens/{id}"),
            key,
            None,
        )
    }
...
mod tests {
    ...
    use crate::spark::wire::{
        OPERATION_SCHEMA, OperationDocument, OperationProgress, OperationState,
    };
    use sha2::Digest;
    use std::{
        fs,
        ...
        thread::spawn(move || {
            for request in 0..5 {
                ...
                let body = if request == 0 {
                    ...
                    serde_json::json!({"schema":"sparkplane.operation/v1","id":OPERATION_ID,"kind":"test","actor_token_id":"admin","target":null,"state":state,"progress":{"stage":"fixture","current":null,"total":null,"unit":null,"message":"waiting"},"created_at":"2026-08-24T00:00:00Z","updated_at":"2026-08-24T00:00:01Z","result":{},"problem":null}).to_string()
                };
                write!(
                    stream,
                    "HTTP/1.1 200 OK\r\nContent-Length: {}\r\nConnection: close\r\n\r\n{}",
    ...
    fn wrong_pin_plaintext_lan_and_missing_token_fail_closed() {
        ...
        .unwrap();
        assert!(
            !SparkClient::load(root.path(), "dgx")
                .unwrap_err()
                .to_string()
                .contains(leaked)
        );
