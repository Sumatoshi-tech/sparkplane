...
use super::wire::{
    MODEL_SCHEMA, ModelArtifactFileDocument, ModelArtifactFormat, ModelArtifactRole,
...
impl ArtifactSelection {
    ...
    pub fn configured(artifacts: ModelArtifactsDocument) -> Result<Self, ModelInputError> {
        if artifacts.schema != "sparkplane.model-artifacts/v2" {
...
mod tests {
    ...
    use super::{
        Alias, CommitSha, ExpectedFile, Repository, Revision, TransferFailure, plan_removal,
        should_run_fallback, verify_snapshot,
    };
    use sha2::Digest;
    use std::{
        fs,
    ...
    fn missing_or_ambiguous_artifact_fails_before_download() {
        ...
        let declared: crate::spark::wire::ModelArtifactsDocument = serde_json::from_str(
            r#"{"schema":"sparkplane.model-artifacts/v2","format":"gguf","primary":{"path":"model.gguf","bytes":8,"sha256":null},"auxiliary":[],"quantization":"Q4","capabilities":["text_generation"],"configured_alias":"model:q4"}"#,
        )
        .unwrap();
    ...
    fn model_identity_changes_with_the_auxiliary_artifact_set() {
        let first: crate::spark::wire::ModelArtifactsDocument = serde_json::from_str(
            r#"{"schema":"sparkplane.model-artifacts/v2","format":"gguf","primary":{"path":"model.gguf","bytes":8,"sha256":null},"auxiliary":[{"role":"projector","path":"vision-a.gguf","bytes":4,"sha256":null}],"quantization":"Q4","capabilities":["text_generation","vision"],"configured_alias":"model:q4"}"#,
        )
        .unwrap();
